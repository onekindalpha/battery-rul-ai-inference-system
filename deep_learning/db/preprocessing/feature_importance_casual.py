"""
feature_importance_causal.py

Step 2: causal DB 기반 물리 피처 중요도 분석 스크립트

- Input : db/battery_training_data_cleaned_final_causal.csv
- Output: 콘솔 출력 + causal_figs/ 에
...
# ---------------------------------------------------------------------
def get_fig_dir():
    fig_dir = Path(__file__).resolve().parent / "causal_figs"
...
def run_feature_importance_for_set(df: pd.DataFrame, feat_cols, tag: str):
    print("\n" + "=" * 80)
    print(f"[CAUSAL] Feature set: {tag}")
    ...
    save_bar_plot(
        ...
        labels=feat_cols,
        title=f"RF Gini Importances (causal, {tag})",
        filename=f"fi_rf_gini_causal_{tag}.png",
    )
    ...
    save_bar_plot(
        values=perm_mean,
        labels=feat_cols,
        title=f"Permutation Importances (causal, {tag})",
        filename=f"fi_perm_causal_{tag}.png",
    )
    ...
    save_bar_plot(
        values=corr_vals,
        labels=feat_cols,
        title=f"|Pearson corr| vs RUL (causal, {tag})",
        filename=f"fi_corr_causal_{tag}.png",
    )
...
def make_common_importance_plot(metrics: dict, tag: str):
    ...
    save_bar_plot(
        values=common_score,
        labels=feat_cols,
        title=f"Common importance (causal, {tag})",
        filename=f"fi_common_causal_{tag}.png",
    )
...
def main():
    print("=" * 80)
    print("[Feature Importance] causal DB 기반 피처 중요도 분석")
    ...
    print("[Info] Loaded causal DB:")
    print(df.head())
    print("[Info] Columns:", df.columns.tolist())
    print(f"[Info] RUL stats ({TARGET_COL}):")
    ...
    feat_cols_full = [c for c in physics_cols if c in df.columns]

    # ---- (B) 상관계수/누출 기반 cleaned feature set 만들기 ----
    # capacity_ahr / discharge_time_sec 같은 직관적 피처는
    # "너무 직결"된 피처로 보고 cleaned set에서만 빼고 비교용으로 남겨둠.
    ...
    if len(feat_cols_full) > 0:
        ...
        save_corr_heatmap(
            corr_full,
            title=f"Correlation heatmap (causal full, target={TARGET_COL})",
            filename="corr_heatmap_causal_full.png",
        )
    ...
    if len(feat_cols_clean) >= 2:
        ...
        save_corr_heatmap(
            corr_clean,
            title=f"Correlation heatmap (causal cleaned, target={TARGET_COL})",
            filename="corr_heatmap_causal_cleaned.png",
        )

    print("\n✅ Done. causal DB에 대해 full / cleaned 중요도 + 공통 중요도 + 히트맵까지 생성 완료.")
