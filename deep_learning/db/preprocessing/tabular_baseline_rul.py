...
#
# NASA + CACLE 공용 RUL 베이스라인 스크립트
#   --dataset nasa | cacle
#   --db basic | causal
#   --model xgb | mlp | both
#   --feature-set full | cleaned
#   --use-ceemdan (causal + CEEMDAN merge)
#   --analyze-rul (MLP 결과 분석 플롯)
#   --extra-exclude "B0032,B0033" (데이터셋 기본 제외 외에 추가 제외)
...
# NASA dataset DBs
BASIC_DB_NASA = DB_DIR / "battery_training_data_cleaned_final_basic.csv"
CAUSAL_DB_NASA = DB_DIR / "battery_training_data_cleaned_final_causal.csv"
CEEMDAN_DB_NASA = DB_DIR / "ceemdan_all_batteries_advanced_causal.csv"

# CACLE dataset DBs
BASIC_DB_CACLE = DB_DIR / "cacle_battery_training_data_cleaned_final_basic.csv"
CAUSAL_DB_CACLE = DB_DIR / "cacle_battery_training_data_cleaned_final_causal.csv"
CEEMDAN_DB_CACLE = DB_DIR / "cacle_ceemdan_all_batteries_advanced_causal.csv"

# (이 아래 BASIC_DB/CAUSAL_DB/CEEMDAN_DB는 혹시 다른 코드가 import 할 때를 위한 alias)
BASIC_DB = BASIC_DB_NASA
CAUSAL_DB = CAUSAL_DB_NASA
CEEMDAN_DB = CEEMDAN_DB_NASA
...
def drop_constant_rul_batteries(
    ...
    (NASA constant-RUL 배터리, CACLE에도 적용)
    """
    bid_col = "battery_id" if "battery_id" in df.columns else "battery"
...
# ---------------------------------------------------------------------
# 3. DB 로드 + 공통 전처리 (NASA / CACLE 공용)
# ---------------------------------------------------------------------
def load_db(dataset: str, db_type: str, use_ceemdan: bool = False, extra_exclude: str = ""):
    """
    dataset:
      - 'nasa'  : NASA DB 사용
      - 'cacle' : CACLE DB 사용
    db_type:
      - 'basic'  : basic DB
      - 'causal' : causal DB (+ 옵션으로 CEEMDAN merge)
    """
    # ---- dataset별 DB 경로 선택 ----
    if dataset == "nasa":
        basic_path = BASIC_DB_NASA
        causal_path = CAUSAL_DB_NASA
        ceemdan_path = CEEMDAN_DB_NASA
    elif dataset == "cacle":
        basic_path = BASIC_DB_CACLE
        causal_path = CAUSAL_DB_CACLE
        ceemdan_path = CEEMDAN_DB_CACLE
    else:
    ...
    if db_type == "basic":
        ...
        print("=" * 80)
        print(f"[Load] dataset={dataset}, DB type=basic, path={path}")
        print("=" * 80)
        ...
        feat_path = causal_path
        if not feat_path.exists():
            raise FileNotFoundError(feat_path)

        print("=" * 80)
        print(f"[Load] dataset={dataset}, DB type=causal, path={feat_path}")
        if use_ceemdan:
            print(f"[Load]   + CEEMDAN features: {ceemdan_path}")
        print("=" * 80)
        ...
        if use_ceemdan:
            if not ceemdan_path.exists():
                raise FileNotFoundError(ceemdan_path)
            ceem = pd.read_csv(ceemdan_path)
    ...
    # basic DB용 thermal_stress 보정 (thermal_stress 없으면 만들어줌)
    if (
    ...
    if "rul_cycles" in df.columns:
        target_col = "rul_cycles"       # CSV에 있으면 최우선
    ...
    # ---- dataset별 default exclude + extra_exclude ----
    bid_col = "battery_id" if "battery_id" in df.columns else "battery"

    if dataset == "nasa":
        # train에서 제외: B0052, B0050 / OOD(meta-test): B0033
        default_exclude = {"B0050", "B0052"}
    elif dataset == "cacle":
        # CACLE에서는 CS2_38 빼고 학습
        default_exclude = set()
    else:
        default_exclude = set()

    extra_set = {s.strip() for s in extra_exclude.split(",") if s.strip()}
    exclude_batteries = default_exclude | extra_set

    if exclude_batteries:
        before = len(df)
        df = df[~df[bid_col].astype(str).isin(exclude_batteries)].reset_index(drop=True)
        after = len(df)
        print(
            f"[Filter] Exclude batteries {sorted(exclude_batteries)}: "
            f"{before} -> {after} rows"
        )

    print("[Info] Final RUL stats after all filters:")
    print(df[target_col].describe())
...
# ---------------------------------------------------------------------
# 4. Baseline models (XGB / MLP, NASA + CACLE 공용)
# ---------------------------------------------------------------------
def run_xgb_baseline(X, y, groups, feature_names, dataset_label=""):
    ...
    tag = f" ({dataset_label})" if dataset_label else ""

    print("\n" + "=" * 80)
    print(f"[XGBoost] GroupKFold cross-validation{tag}")
    print("=" * 80)
...
def run_mlp_baseline(X, y, groups, feature_names, dataset_label=""):
    n_groups = len(np.unique(groups))
    n_splits = min(5, n_groups)
    gkf = GroupKFold(n_splits=n_splits)
    ...
    tag = f", {dataset_label}" if dataset_label else ""
    tag_paren = f" ({dataset_label})" if dataset_label else ""

    print("\n" + "=" * 80)
    print(f"[MLP] GroupKFold cross-validation{tag_paren}")
    print("=" * 80)
    ...
    # 1) fold 평균
    print(f"\n[MLP] CV summary (mean of folds{tag}):")
    print(f"  RMSE: {np.mean(rmses):.3f} ± {np.std(rmses):.3f}")
    print(f"  MAE : {np.mean(maes):.3f} ± {np.std(maes):.3f}")
    print(f"  R2  : {np.mean(r2s):.3f} ± {np.std(r2s):.3f}")
    ...
    print(f"\n[MLP] GLOBAL metrics (all folds concatenated{tag}):")
    ...
    print(f"\n[MLP] Dummy-mean baseline (GLOBAL{tag}):")
    ...
    print(f"\n[MLP] Improvement over dummy-mean (GLOBAL{tag}):")
    ...
    print("\n[MLP] Per-battery error (top 5 by MAE" + (f", {dataset_label}" if dataset_label else "") + "):")
...
def analyze_rul_predictions(y_true_all, y_pred_all, groups_all,
    ...
    # 1) 구간별 metric
    print_rul_binned_metrics(y_true_all, y_pred_all, dataset_label=dataset_label)
    ...
    for bid, n, mae_b, rmse_b, m_rul in stats_sorted[:top_k]:
        ...
        plt.tight_layout()
        plt.savefig(output_dir / f"scatter_true_vs_pred_{dataset_label}_{bid}.png", dpi=200)
        plt.close()
...
# ---------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Tabular RUL baselines on basic/causal DB (NASA + CACLE)")
    parser.add_argument("--dataset", choices=["nasa", "cacle"], default="nasa",
                        help="사용할 데이터셋 (nasa / cacle)")
    ...
    parser.add_argument("--extra-exclude", type=str, default="",
                        help="추가로 제외할 배터리 id들을 콤마로 구분해서 입력 (예: B0032,B0033)")
    args = parser.parse_args()

    # 1) DB 로드 + RUL(target) 결정
    df, target_col = load_db(args.dataset, args.db, use_ceemdan=args.use_ceemdan,
    ...
    print("=" * 80)
    print(f"[Info] dataset={args.dataset}, DB={args.db}, feature_set={args.feature_set}, target={target_col}")
    ...
    if args.model in ("xgb", "both"):
        run_xgb_baseline(X, y, groups, feature_cols, dataset_label=dataset_label)

    mlp_result = None
    if args.model in ("mlp", "both"):
        mlp_result = run_mlp_baseline(X, y, groups, feature_cols, dataset_label=dataset_label)

    # 분석 호출 (플래그가 켜져 있을 때만)
    if args.analyze_rul and mlp_result is not None:
        out_dir = FILE_PATH.parent / f"rul_analysis_plots_{args.dataset}_both"
...
# python "(preprocessing)tabular_baseline_rul_both.py" \
#   --dataset nasa \
#   --db causal \
#   --model both \
#   --feature-set full \
