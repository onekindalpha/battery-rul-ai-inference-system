...
# --- Load / Voltage ---
    ...
    with right:
        ...
        if all_cycles.size == 0 or all_true.size == 0:
            ...
            # Current true point
            if np.isfinite(current_true_rul):
                fig_rul.add_trace(
                    go.Scatter(
            ...
            # Current predicted point (only if in pred region)
            if np.isfinite(current_pred_rul):
                fig_rul.add_trace(
                    go.Scatter(
                        x=[float(current_cycle)],
            ...
            # Mini panel: Δ Predicted EOL (how forecast shifts)
            if fut_cycles.size > 1 and fut_rul_pred.size > 1:
...
"
            f"- Pred RUL (cycles): **{current_pred_rul if not math.isnan(current_pred_rul) else 'N/A'}**
"
            f"- Risk: **{risk_lbl}**
...
"
            f"- Severity (max |z|): **{main_sev if not math.isnan(main_sev) else 'N/A'}**
"
...
"
            f"## Notes
"
            f"- z-score definition: z = (x - median(cycle)) / IQR(cycle)
"
            f"- Bands: median ± IQR (Q1–Q3)
"
        )
