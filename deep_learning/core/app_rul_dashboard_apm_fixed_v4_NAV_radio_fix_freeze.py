...
NASA_FEATURES_PATH = next((p for p in NASA_FEATURES_PATH_CANDIDATES if p.exists()), FILE_DIR / "analysis" / "nasa_features_rul.csv")
...
def load_cycle_features():
    ...
    # ensure types for exact matching
...
# Main content (tabs)
# =================================================
page = st.sidebar.radio("Page", ["Overview", "Monitoring", "Compare / Fleet", "What-if"], index=0)
...
if page == "Overview":
    ...
    with left:
        ...
        if cycle_ctx.row is None:
            st.info("No cycle-feature row found for this battery/cycle in cycle-feature table.")
        else:
            row = cycle_ctx.row
            ...
            # requested UI hints
            discharge_current_class_v = str(row.get("discharge_current_class", "N/A")) if row is not None else "N/A"
            exp_cond_v = str(row.get("experiment_condition", "N/A")) if row is not None else "N/A"


            a, b = st.columns(2)
            with a:
                st.metric("discharge_current_class (1A/2A/4A)", discharge_current_class_v)
            ...
            with b:
                st.metric("실험조건 (실온/4도)", exp_cond_v)
