...
SCENARIO_FEATURES = {
    ...
    "regen_strength": {
        "label": "Cumulative capacity loss (vs peak)",
        "fallback_min": 0.0,
        "fallback_max": 1.5,
        "step": 0.05,
    },
    ...
    "temp_rise_cycle": {
        "label": "Cycle temperature rise (cell − ambient, °C)",
        "fallback_min": 0.0,
        "fallback_max": 20.0,
        "step": 0.5,
    },
...
# -----------------------------
with st.sidebar.expander("Notifications (Slack / Email)", expanded=False):
    ...
    smtp_user = st.text_input("SMTP user (from)", value=st.session_state.get("smtp_user", ""), key="smtp_user", disabled=not email_enabled)
    smtp_password = st.text_input("SMTP password", value=st.session_state.get("smtp_password", ""), key="smtp_password", type="password", disabled=not email_enabled)
    to_email = st.text_input("To email", value=st.session_state.get("to_email", ""), key="to_email", disabled=not email_enabled)
    ...
    if st.button("Send test notification", disabled=not enable_notif):
        ...
        if slack_webhook_url:
            ok_s, msg_s = _send_slack(slack_webhook_url, test_msg)
        if email_enabled:
            ok_e, msg_e = _send_email_smtp(
                smtp_host=str(smtp_host),
                smtp_port=int(smtp_port),
                smtp_user=str(smtp_user),
                smtp_password=str(smtp_password),
                to_email=str(to_email),
                subject="RUL dashboard test",
                body=test_msg,
                use_tls=True,
            )
        st.write({"slack": {"ok": ok_s, "msg": msg_s}, "email": {"ok": ok_e, "msg": msg_e}})

# 아직 records가 전혀 없으면 안내 후 종료
if "records" not in st.session_state or not st.session_state["records"]:
    st.info(
        "Use the left sidebar **Initialize model & run inference** button to load data.\n\n"
...
st.sidebar.header("2) 테스트 배터리 및 Cycle 선택")
...
# 🔁 Auto-play 체크박스
auto_play = st.sidebar.checkbox("Auto-play cycles", key="auto_play", value=False)
...
st.sidebar.subheader("Model prediction error metrics")
...
# 1Cycle당 주행거리(km) 가정
km_per_cycle = st.session_state.get("km_per_cycle", 400.0)  # 기본 400 km/Cycle (참고용 가정)
...
# Notify on risk escalation (WARN/ALERT)
if st.session_state.get("enable_notif", False) and risk_lbl in ("WARN", "ALERT"):
    last_risk = st.session_state.get("last_notified_risk")
    last_cycle = st.session_state.get("last_notified_cycle")
    # avoid spamming: notify only when (risk changes) or (cycle changes)
    if (last_risk != risk_lbl) or (last_cycle != int(current_cycle)):
        title = f"[RUL Dashboard] {risk_lbl} for {selected_bid} @ cycle {int(current_cycle)}"
        msg = (
            f"{title}\n"
            f"- Remaining RUL: {current_pred_rul:.1f} cycles\n"
            f"- Remaining RUL % of initial: {rul_pct:.1f}%\n"
            f"- Source: {st.session_state.get('records_source','')}"
        )
        slack_url = st.session_state.get("slack_webhook_url", "")
        if slack_url:
            _send_slack(slack_url, msg)
        if st.session_state.get("email_enabled", False):
            _send_email_smtp(
                smtp_host=str(st.session_state.get("smtp_host", "")),
                smtp_port=int(st.session_state.get("smtp_port", 587)),
                smtp_user=str(st.session_state.get("smtp_user", "")),
                smtp_password=str(st.session_state.get("smtp_password", "")),
                to_email=str(st.session_state.get("to_email", "")),
                subject=title,
                body=msg,
                use_tls=True,
            )
        st.session_state["last_notified_risk"] = risk_lbl
        st.session_state["last_notified_cycle"] = int(current_cycle)
# Current uncertainty at cursor (±2σ)
        ...
        st.markdown("### Degradation snapshot")
        if cycle_ctx.row is None:
            ...
            soh_v = _get("soh")
            cap_pct_v = float("nan")
            if df_feat_all is not None and "cap_pct" in df_feat_all.columns:
                sub_b = df_feat_all[df_feat_all["battery_id"] == str(selected_bid)].sort_values("cycle")
                sub_now = sub_b[sub_b["cycle"] == float(current_cycle)]
                if not sub_now.empty:
                    cap_pct_v = _safe_float(sub_now.iloc[-1]["cap_pct"])

            dcr_v = _get("dcr")
            imp_v = _get("impedance_sum")
            temp_v = _get("temperature_mean")
            stress_v = _get("thermal_stress")

            a, b = st.columns(2)
            with a:
                st.metric("SoH", f"{soh_v*100:.1f}%" if not math.isnan(soh_v) else "N/A")
                st.metric("Cap% (vs init)", f"{cap_pct_v:.1f}%" if not math.isnan(cap_pct_v) else "N/A")
                st.metric("DCR", f"{dcr_v:.4f} Ω" if not math.isnan(dcr_v) else "N/A")
            with b:
                st.metric("Impedance", f"{imp_v:.4f} Ω" if not math.isnan(imp_v) else "N/A")
                st.metric("Temp mean", f"{temp_v:.1f} °C" if not math.isnan(temp_v) else "N/A")
                st.metric("Thermal stress", f"{stress_v:.3f}" if not math.isnan(stress_v) else "N/A")

        st.markdown("### Quick anomaly view (z-scores)")
        if df_feat_all is None or df_feat_all.empty:
            ...
            if df_b.empty:
                st.info("No cycle-feature rows for this battery.")
            else:
                # z(DCR), z(Cap%) vs cycle median/IQR
                z_items = []
                if "dcr" in df_feat_all.columns:
                    band_dcr = _cycle_band_table(df_feat_all, "dcr")
                    z_dcr = _zscore_against_cycle_band(df_b, band_dcr, "dcr")
                    df_b = df_b.assign(z_dcr=z_dcr.values)
                    z_items.append(("z(DCR)", "z_dcr"))
                if "cap_pct" in df_feat_all.columns:
                    band_cap = _cycle_band_table(df_feat_all, "cap_pct")
                    z_cap = _zscore_against_cycle_band(df_b, band_cap, "cap_pct")
                    df_b = df_b.assign(z_cap=z_cap.values)
                    z_items.append(("z(Cap%)", "z_cap"))

                # Show current-cycle z values
                df_now = df_b[df_b["cycle"] == float(current_cycle)]
                if not df_now.empty and z_items:
                    vals = {}
                    for label, col in z_items:
                        v = _safe_float(df_now.iloc[-1][col])
                        vals[label] = v
                    # small table
                    st.dataframe(pd.DataFrame([vals]), use_container_width=True, hide_index=True)

    # --- Right: RUL trajectory (bigger)
    with right:
    ...
    if df_feat_all is None or df_feat_all.empty:
        ...
        if df_b.empty:
            ...
            if "soh" in df_b.columns and "soh" in bands:
                ...
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["q3"], mode="lines", line=dict(width=0), fill="tonexty", name="SoH expected band (IQR)", fillcolor="rgba(0,0,0,0.10)"), row=1, col=1)
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["median"], mode="lines", name="SoH expected (median)", line=dict(color="#444444")), row=1, col=1)
                figm.add_trace(go.Scatter(x=df_b["cycle"], y=df_b["soh"], mode="lines", name="SoH (cell)", line=dict(color="#000000")), row=1, col=1)

            if "cap_pct" in df_b.columns and "cap_pct" in bands:
                band = bands["cap_pct"]
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["q1"], mode="lines", line=dict(width=0), showlegend=False, hoverinfo="skip"), row=1, col=1)
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["q3"], mode="lines", line=dict(width=0), fill="tonexty", name="Cap% expected band (IQR)", fillcolor="rgba(0,0,0,0.10)"), row=1, col=1)
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["median"], mode="lines", name="Cap% expected (median)", line=dict(color="#777777")), row=1, col=1)
                figm.add_trace(go.Scatter(x=df_b["cycle"], y=df_b["cap_pct"], mode="lines", name="Cap% (cell)", line=dict(color="#111111")), row=1, col=1)
            ...
            if "dcr" in df_b.columns and "dcr" in bands:
                ...
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["q3"], mode="lines", line=dict(width=0), fill="tonexty", name="DCR expected band (IQR)", fillcolor="rgba(0,0,0,0.10)"), row=2, col=1)
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["median"], mode="lines", name="DCR expected (median)", line=dict(color="#444444")), row=2, col=1)
                figm.add_trace(go.Scatter(x=df_b["cycle"], y=df_b["dcr"], mode="lines", name="DCR (cell)", line=dict(color="#000000")), row=2, col=1)

            if "impedance_sum" in df_b.columns and "impedance_sum" in bands:
                band = bands["impedance_sum"]
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["q1"], mode="lines", line=dict(width=0), showlegend=False, hoverinfo="skip"), row=2, col=1)
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["q3"], mode="lines", line=dict(width=0), fill="tonexty", name="Impedance expected band (IQR)", fillcolor="rgba(0,0,0,0.10)"), row=2, col=1)
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["median"], mode="lines", name="Impedance expected (median)", line=dict(color="#777777")), row=2, col=1)
                figm.add_trace(go.Scatter(x=df_b["cycle"], y=df_b["impedance_sum"], mode="lines", name="Impedance (cell)", line=dict(color="#111111")), row=2, col=1)
            ...
            if "temperature_mean" in df_b.columns and "temperature_mean" in bands:
                ...
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["q3"], mode="lines", line=dict(width=0), fill="tonexty", name="Temp expected band (IQR)", fillcolor="rgba(0,0,0,0.10)"), row=3, col=1, secondary_y=False)
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["median"], mode="lines", name="Temp expected (median)", line=dict(color="#444444")), row=3, col=1, secondary_y=False)
            ...
            if "thermal_stress" in df_b.columns and "thermal_stress" in bands:
                ...
                figm.add_trace(go.Scatter(x=band["cycle"], y=band["median"], mode="lines", name="Thermal stress expected (median)", line=dict(color="#777777")), row=3, col=1, secondary_y=True)
                figm.add_trace(go.Scatter(x=df_b["cycle"], y=df_b["thermal_stress"], mode="lines", name="Thermal stress (cell)", line=dict(color="#111111")), row=3, col=1, secondary_y=True)

            # Cursor vline
            for r in [1, 2, 3]:
                figm.add_vline(x=float(current_cycle), line_width=1, line_dash="dash", line_color="red", row=r, col=1)

            figm.update_layout(height=720, margin=dict(l=40, r=20, t=10, b=30), colorway=["#000000","#444444","#777777","#aaaaaa"])
            ...
            for col in [c for c in z_df.columns if c.startswith("z_")]:
                figz.add_trace(go.Scatter(x=z_df["cycle"], y=z_df[col], mode="lines", name=col, line=dict(color="#000000" if col=="z_dcr" else ("#555555" if col=="z_cap_pct" else "#aaaaaa"))))
            figz.add_hline(y=2.0, line_dash="dash", line_color="gray")
            figz.add_hline(y=-2.0, line_dash="dash", line_color="gray")
            figz.add_vline(x=float(current_cycle), line_dash="dash", line_color="red")
            figz.update_layout(height=280, xaxis_title="Cycle", yaxis_title="z = (x - median)/IQR", colorway=["#000000","#888888"])
