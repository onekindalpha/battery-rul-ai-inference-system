...
with tab_overview:
    ...
    with left:
        ...
        with m3:
            st.markdown(
                ...
                <div style="border:1px solid #e6e6e6; border-radius:10px; padding:8px 10px; text-align:center;">
                  <div style="font-size:11px; color:#666; line-height:1.2;">Risk<br>(rul_pct = 100×PredRUL/InitRUL)</div>
        ...
        if cycle_ctx.row is None:
            ...
            temperature_measured_max_v = _get_any(["temperature_measured_max", "temperature_max", "temperature_mean"])
            ...
            with c1:
                st.markdown(
                    f"""
                    <div style="border:1px solid #e6e6e6; border-radius:10px; padding:8px 10px;">
                      <div style="font-size:11px; color:#666; line-height:1.2;">capacity_ahr<br>(end-of-discharge capacity, Ah)</div>
                      <div style="font-size:16px; font-weight:700;">{(f"{capacity_ahr_v:.3f}" if np.isfinite(capacity_ahr_v) else "N/A")}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
            with c2:
                st.markdown(
                    f"""
                    <div style="border:1px solid #e6e6e6; border-radius:10px; padding:8px 10px;">
                      <div style="font-size:11px; color:#666; line-height:1.2;">soh<br>(capacity(% initial)</div>
                      <div style="font-size:16px; font-weight:700;">{(f"{soh_v:.3f}" if np.isfinite(soh_v) else "N/A")}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
            ...
            with l1:
                st.metric("voltage_min", f"{voltage_min_v:.3f}" if np.isfinite(voltage_min_v) else "N/A")
            with l2:
                st.markdown(
                    f"""
                    <div style="border:1px solid #e6e6e6; border-radius:10px; padding:8px 10px;">
                      <div style="font-size:11px; color:#666; line-height:1.2;">eff_c_rate<br>(capacity_ahr / (discharge_time_sec/3600) (단위 A))</div>
                      <div style="font-size:16px; font-weight:700;">{(f"{eff_c_rate_v:.3f}" if np.isfinite(eff_c_rate_v) else "N/A")}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
            with l3:
                st.markdown(
                    f"""
                    <div style="border:1px solid #e6e6e6; border-radius:10px; padding:8px 10px; text-align:center;">
                      <div style="font-size:11px; color:#666;">discharge_current_class</div>
                      <div style="font-size:16px; font-weight:700;">{discharge_current_class_v}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
            ...
            t1, t2 = st.columns(2)
            with t1:
                st.metric("실험 조건", exp_cond)
            with t2:
                st.metric("temperature_measured_max", f"{temperature_measured_max_v:.2f}" if np.isfinite(temperature_measured_max_v) else "N/A")

            st.markdown("#### Resistance")
            r1, r2 = st.columns(2)
            with r1:
                st.metric("dcr(Re)", f"{dcr_re_v:.4f}" if np.isfinite(dcr_re_v) else "N/A")
            with r2:
                st.markdown(
                    f"""
                    <div style="border:1px solid #e6e6e6; border-radius:10px; padding:8px 10px;">
                      <div style="font-size:11px; color:#666; line-height:1.2;">dcr growth<br>(Internal resistance increase (log ratio vs early life)</div>
                      <div style="font-size:16px; font-weight:700;">{(f"{dcr_growth_v:.4f}" if np.isfinite(dcr_growth_v) else "N/A")}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
        ...
        if df_feat_all is None or df_feat_all.empty:
            ...
            if df_b.empty:
                ...
                # z(DCR), z(Capacity) vs cycle median/IQR
                z_items = []
                ...
                cap_col = "capacity_ahr" if "capacity_ahr" in df_feat_all.columns else ("capacity_mean" if "capacity_mean" in df_feat_all.columns else None)
                if cap_col is not None:
                    band_cap = _cycle_band_table(df_feat_all, cap_col)
                    z_cap = _zscore_against_cycle_band(df_b, band_cap, cap_col)
                    df_b = df_b.assign(z_cap=z_cap.values)
                    z_items.append(("z(Capacity)", "z_cap"))
