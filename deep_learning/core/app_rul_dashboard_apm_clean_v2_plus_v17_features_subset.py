...
with tab_overview:
    ...
    with left:
        ...
        if cycle_ctx.row is None:
            ...
            def _get_str(key, default="N/A"):
                try:
                    v = row.get(key, None) if row is not None else None
                except Exception:
                    v = None
                if v is None:
                    return default
            ...
            discharge_current_class_v = _get_str("discharge_current_class", default="N/A")

            # discharge_current_class fallback: infer from |current_min| → 1A/2A/4A
            if discharge_current_class_v == "N/A":
            ...
            # c_ref_ahr fallback: per-battery early-life median of capacity_ahr (first k cycles)
            if (not np.isfinite(c_ref_ahr_v)) and df_feat_all is not None and (not df_feat_all.empty) and ("capacity_ahr" in df_feat_all.columns):
                try:
                    sub_b = df_feat_all[df_feat_all["battery_id"] == str(selected_bid)].sort_values("cycle")
                    if len(sub_b) > 0:
                        k_ref = 10
                        c_ref_ahr_v = float(np.nanmedian(sub_b["capacity_ahr"].astype(float).values[:k_ref]))
                except Exception:
                    pass
            ...
            # --- Temperature (requested 3) ---
            temp_start_v = _get_any([
                "temperature_measured_0",
                "temperature_measured_start",
                "temp_start",
                "temperature_start",
                "ambient_temp_c",
                "ambient_temp",
            ])
            temp_max_v = _get_any(["temperature_measured_max", "temperature_max", "temp_max"])
            temp_rise_cycle_v = _get_any(["temp_rise_cycle"])
            ...
            if (not np.isfinite(temp_rise_cycle_v)) and np.isfinite(temp_max_v) and np.isfinite(temp_start_v):
                temp_rise_cycle_v = float(temp_max_v - temp_start_v)

            # thermal_stress fallback: exp(alpha * (Tmax - Tref)), where Tref = early-life median of Tmax
            if (not np.isfinite(thermal_stress_v)) and np.isfinite(temp_max_v) and df_feat_all is not None and (not df_feat_all.empty):
                try:
                    alpha = 0.05
                    if "temperature_measured_max" in df_feat_all.columns:
                        sub_b = df_feat_all[df_feat_all["battery_id"] == str(selected_bid)].sort_values("cycle")
                        k_ref = 10
                        tref = float(np.nanmedian(sub_b["temperature_measured_max"].astype(float).values[:k_ref])) if len(sub_b) else float("nan")
                        if np.isfinite(tref):
                            thermal_stress_v = float(np.exp(alpha * (float(temp_max_v) - tref)))
                except Exception:
                    pass

            # --- Resistance (requested 2) ---
            dcr_re_v = _get_any(["dcr", "re_ohm_interp", "Re", "re"])
            dcr_growth_v = _get_any(["dcr_growth_log", "dcr_growth"])

            # dcr_growth fallback: log((Re+eps)/(Re_ref0+eps)) from early-life baseline
            if (not np.isfinite(dcr_growth_v)) and np.isfinite(dcr_re_v) and df_feat_all is not None and (not df_feat_all.empty):
                try:
                    sub_b = df_feat_all[df_feat_all["battery_id"] == str(selected_bid)].sort_values("cycle")
                    base_col = "dcr" if "dcr" in sub_b.columns else None
                    if base_col is not None and len(sub_b) > 0:
                        k_ref = 10
                        re_ref0 = float(np.nanmedian(sub_b[base_col].astype(float).values[:k_ref]))
                        eps = 1e-9
                        if np.isfinite(re_ref0):
                            dcr_growth_v = float(np.log((float(dcr_re_v) + eps) / (re_ref0 + eps)))
                except Exception:
                    pass
            ...
            with l1:
                st.markdown(
                    ...
                    <div style="border:1px solid #e6e6e6; border-radius:12px; padding:10px 12px; min-height:78px; text-align:center;">
                      <div style="font-size:11px; color:#666; line-height:1.2;">discharge_current_<br>class (1A/2A/4A)</div>
            ...
            with t1:
                _metric_card(
                    ...
                    f"{temp_start_v:.2f}°C" if np.isfinite(temp_start_v) else "N/A",
                )
            with t2:
                _metric_card(
                    "temp_rise_cycle<br>(temperature_measured_max - Temperature_measured[0], °C)",
                    f"{temp_rise_cycle_v:.2f}°C" if np.isfinite(temp_rise_cycle_v) else "N/A",
                )
            with t3:
                _metric_card(
                    "thermal_stresss<br>(exp(alpha * (Tmax - Tref)))",
