...
# Autoplay (no interval slider)
if "autoplay" not in st.session_state:
    st.session_state["autoplay"] = False

col_ap1, col_ap2 = st.sidebar.columns([1,1])
with col_ap1:
    if st.button("▶ Auto-play" if not st.session_state["autoplay"] else "⏸ Pause", key="btn_autoplay"):
        st.session_state["autoplay"] = not st.session_state["autoplay"]
with col_ap2:
    if st.button("⏭ Step", key="btn_step"):
        st.session_state["play_cycle"] = min(max_cycle, int(st.session_state["play_cycle"]) + 1)

if st.session_state["autoplay"] and st_autorefresh is not None:
    st_autorefresh(interval=1200, key="autoplay_refresh")
    # advance one step per refresh
    st.session_state["play_cycle"] = min(max_cycle, int(st.session_state["play_cycle"]) + 1)
st.sidebar.caption(
...
if page == "Overview":
    ...
    lv9, lv10, lv11, lv12 = st.columns(4, gap="small")
    with lv9:
        _metric_card("c_rate_mean = abs(current_mean)<br>/ c_ref_ahr", f"{c_rate_mean_v:.3f} C" if np.isfinite(c_rate_mean_v) else "—")
    with lv10:
