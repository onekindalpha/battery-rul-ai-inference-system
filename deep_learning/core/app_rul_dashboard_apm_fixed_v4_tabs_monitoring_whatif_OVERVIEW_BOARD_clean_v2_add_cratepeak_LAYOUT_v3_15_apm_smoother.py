...
NASA_FEATURES_PATH_CANDIDATES = [
    # Fast default (smaller / stable)
    SCRIPT_DIR / "analysis" / "nasa_features_rul.csv",
    SCRIPT_DIR / "nasa_features_rul.csv",
    CWD / "analysis" / "nasa_features_rul.csv",
    CWD / "nasa_features_rul.csv",

    # Enriched (optional / may be heavier)
...
# --- Per-battery reference capacity map (for C-rate metrics) ---
BATTERY_META_C_REF_CANDIDATES = [
    SCRIPT_DIR / "analysis" / "battery_meta_c_ref.json",
    SCRIPT_DIR / "battery_meta_c_ref.json",
    CWD / "analysis" / "battery_meta_c_ref.json",
    CWD / "battery_meta_c_ref.json",
    Path("/Users/velocitygoal/Desktop/battery_project/v11/server/backend/deep_learning/core/analysis/battery_meta_c_ref.json"),
]
BATTERY_META_C_REF_PATH = _first_existing(BATTERY_META_C_REF_CANDIDATES)

@st.cache_data(show_spinner=False)
def _load_c_ref_meta() -> dict:
    if BATTERY_META_C_REF_PATH is None:
        return {}
    try:
        with open(BATTERY_META_C_REF_PATH, "r") as f:
            return json.load(f)
    except Exception:
        return {}

@st.cache_data(show_spinner=False)
def get_c_ref_ahr_map() -> dict:
    meta = _load_c_ref_meta()
    m = meta.get("c_ref_ahr_by_battery", {})
    return m if isinstance(m, dict) else {}

@st.cache_data(show_spinner=False)
def get_c_ref_params() -> tuple:
    meta = _load_c_ref_meta()
    k_ref = int(meta.get("k_ref", 10))
    cap_col = str(meta.get("cap_col", "capacity_mean"))
    cap_min = float(meta.get("cap_min", 0.2))
    return k_ref, cap_col, cap_min
...
def run_bmaml_once(r_ratio: float, note: str = ""):
    ...
    with loading_placeholder.container():
        ...
        # NOTE: GIF removed to avoid media cache issues during reruns
...
# Memoize per-battery reference capacity for C-rate metrics (useMemo-like)
if st.session_state.get("_c_ref_bid") != selected_bid:
    c_map = get_c_ref_ahr_map()
    c_ref = c_map.get(str(selected_bid), float("nan"))
    try:
        c_ref = float(c_ref) if c_ref is not None else float("nan")
    except Exception:
        c_ref = float("nan")
    st.session_state["_c_ref_bid"] = str(selected_bid)
    st.session_state["_c_ref_ahr"] = c_ref
...
# 🔁 Auto-play 체크박스
# NOTE: widget key(auto_play)는 생성된 뒤에는 session_state에서 직접 수정할 수 없음.
# auto-play 루프가 종료 조건에 걸리면 _auto_play_force_stop 플래그를 세팅하고,
# 다음 rerun에서(체크박스 생성 전에) 여기서 auto_play를 False로 내려준다.
if st.session_state.get("_auto_play_force_stop", False):
    st.session_state["_auto_play_force_stop"] = False
...
# --- Load / Voltage ---
            ...
            # c_ref_ahr: prefer column if present; otherwise use per-battery memoized meta (fast, no DB scan)
            if (not np.isfinite(c_ref_ahr_v)):
                try:
                    c_ref_ahr_v = float(st.session_state.get("_c_ref_ahr", float("nan")))
                except Exception:
                    c_ref_ahr_v = float("nan")
            ...
            # --- Temperature ---
            temp_start_c_v = _get_any(["ambient_temp_c", "ambient_temperature", "ambient_temp", "temp_measured_0_c"])
            temperature_measured_max_v = _get_any(["temperature_measured_max", "temperature_max"])
            temp_rise_cycle_v = _get_any(["temp_rise_cycle", "temp_rise"])
            thermal_stress_v = _get_any(["thermal_stress", "thermal_stresss"])

            # temp_rise_cycle fallback: Tmax - T0
            if (not np.isfinite(temp_rise_cycle_v)) and np.isfinite(temperature_measured_max_v) and np.isfinite(temp_start_c_v):
                temp_rise_cycle_v = float(temperature_measured_max_v - temp_start_c_v)
            ...
            def _metric_card(label_html: str, value_html: str):
                st.markdown(
                    f"""
                    <div style="border:1px solid #e6e6e6; border-radius:12px; padding:10px 12px; min-height:72px;">
                      <div style="font-size:11px; color:#666; line-height:1.2;">{label_html}</div>
                      <div style="font-size:20px; font-weight:700; margin-top:6px; line-height:1.0; white-space:nowrap; display:flex; align-items:baseline; gap:4px;">{value_html}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
            ...
            with c1:
                _metric_card(
                    ...
                    f"{capacity_ahr_v:.3f}" + "<span style='font-size:13px; font-weight:600; margin-left:4px;'>Ah</span>" if np.isfinite(capacity_ahr_v) else "N/A",
                )
            with c2:
                _metric_card(
                    "soh<br>(capacity(% initial))",
                    f"{(soh_v*100.0):.1f}" + "<span style='font-size:13px; font-weight:600; margin-left:4px;'>%</span>" if np.isfinite(soh_v) else "N/A",
                )

            
            st.markdown("#### ⚡️ Load/Voltage")
            ...
            with lv1:
                st.markdown(
                    ...
                    <div style="border:1px solid #e6e6e6; border-radius:12px; padding:10px 12px; min-height:78px; text-align:center;">
                      <div style="font-size:11px; color:#666; line-height:1.2;">discharge_current_class<br>(1A/2A/4A)</div>
            ...
            with lv2:
                _metric_card(
                    "current_mean (A)",
                    f"{current_mean_v:.3f}" + "<span style='font-size:13px; font-weight:600; margin-left:4px;'>A</span>" if np.isfinite(current_mean_v) else "N/A",
                )
            with lv3:
                _metric_card(
                    "c_rate_peak (C)<br>= abs(current_min) / c_ref_ahr",
                    f"{c_rate_peak_v:.3f}" + "<span style='font-size:13px; font-weight:600; margin-left:4px;'>C</span>" if np.isfinite(c_rate_peak_v) else "N/A",
                )
            ...
            with lv4:
                _metric_card(
                    ...
                    f"{voltage_measured_mean_v:.3f}" + "<span style='font-size:13px; font-weight:600; margin-left:4px;'>V</span>" if np.isfinite(voltage_measured_mean_v) else "N/A",
                )
            with lv5:
                _metric_card(
                    "voltage_min (V)",
                    f"{voltage_min_v:.3f}" + "<span style='font-size:13px; font-weight:600; margin-left:4px;'>V</span>" if np.isfinite(voltage_min_v) else "N/A",
                )

            
            st.markdown("#### 🌡️ Temperature")

            # Put thermal_stress on top (APM-like)

            t1, t2 = st.columns(2)
            with t1:
                _metric_card(
                    "(Temperature_measured[0] at<br>discharge start, °C)",
                    (f"{temp_start_c_v:.2f}" + "<span style='font-size:13px; font-weight:600; margin-left:4px;'>°C</span>" if np.isfinite(temp_start_c_v) else "N/A"),
                )
            with t2:
                _metric_card(
                    "temp_rise_cycle<br>(Tmax - Temperature_measured[0], °C)",
                    (f"{temp_rise_cycle_v:.2f}" + "<span style='font-size:13px; font-weight:600; margin-left:4px;'>°C</span>" if np.isfinite(temp_rise_cycle_v) else "N/A"),
                )


            _metric_card(
                "thermal_stress<br>(exp(alpha * (Tmax - Tref)))",
                f"{thermal_stress_v:.3f}" if np.isfinite(thermal_stress_v) else "N/A",
            )
    ...
    # --- Right: RUL trajectory ---
    with right:
        st.markdown("### 📈 Predicted RUL trajectory")
        if all_cycles.size == 0 or all_true.size == 0:
            st.info("RUL trajectory data is not available for this battery.")
        else:
            fig_rul = go.Figure()

            # Observed/true RUL across all cycles
            fig_rul.add_trace(
                go.Scatter(
                    x=all_cycles,
                    y=all_true,
            ...
            # Prediction region (future) with uncertainty band
            if fut_cycles.size > 0 and fut_rul_pred.size > 0:
                if fut_rul_std.size == fut_rul_pred.size and fut_rul_std.size > 0:
                    lower = fut_rul_pred - 2.0 * fut_rul_std
                    upper = fut_rul_pred + 2.0 * fut_rul_std

                    # band (lower first, then upper filled to next y)
                    fig_rul.add_trace(
                        go.Scatter(
                            x=fut_cycles,
                            y=lower,
            ...
            # Cursor and points
            try:
                fig_rul.add_vline(x=float(current_cycle), line_width=1, line_dash="dash")
            except Exception:
                pass

            # Selected cycle markers: show true+pred together only when prediction exists at this cycle
            show_selected = bool(np.isfinite(current_pred_rul) and np.isfinite(current_true_rul))
            if show_selected:
                fig_rul.add_trace(
                    go.Scatter(
                        x=[float(current_cycle)],
                        y=[float(current_true_rul)],
                        mode="markers",
            ...
            st.caption("Selected-cycle markers are shown only when prediction is available at that cycle.")

            # Optional: EOL shift (Δpred_EOL) — helps see when the forecast moves earlier/later
            show_eol_shift = st.checkbox("Show EOL shift (Δpred_EOL)", value=False, key="show_eol_shift")
            if show_eol_shift and fut_cycles.size > 1 and fut_rul_pred.size > 1:
                pred_eol = fut_cycles + fut_rul_pred
                # delta at each point (first point delta=0)
                delta = np.diff(pred_eol, prepend=pred_eol[0])
                ...
                fig_deol.add_hline(y=0.0, line_dash="dash")
                fig_deol.add_vline(x=float(current_cycle), line_dash="dash")
                fig_deol.update_layout(
                    height=180,
                ...
                st.plotly_chart(fig_deol, use_container_width=True)
                st.caption("Δpred_EOL = pred_EOL(t) − pred_EOL(t−1).  Negative → earlier EOL (worse), Positive → later EOL (better).")


# In auto-play mode, pause heavy tabs to reduce rerun load and UI flicker.
# The dashboard remains interactive; turn off auto-play to use Monitoring/What-if.
if st.session_state.get("auto_play", False):
    st.info("Auto-play mode is ON — Monitoring/What-if tabs are paused to keep playback smooth.")
    st.stop()
...
# Report export (generate on demand; prevents MediaFileHandler churn during autoplay reruns)
if "report_cache" not in st.session_state:
    st.session_state["report_cache"] = {}

with st.expander("Report export", expanded=False):
    autoplay_on = bool(st.session_state.get("auto_play", False))
    if autoplay_on:
        st.caption("Pause autoplay to generate exports (avoids heavy reruns).")

    gen_key = (selected_bid, int(current_cycle))
    if st.button("Generate report for current cycle", key=f"gen_report_{selected_bid}_{int(current_cycle)}", disabled=autoplay_on):
...
'''
        cache_item = {"md": report_md.encode("utf-8"), "pdf": None, "cycle": int(current_cycle)}
        ...
        try:
            ...
            c.showPage()
            c.save()
            cache_item["pdf"] = buf.getvalue()
        except Exception:
            cache_item["pdf"] = None

        st.session_state["report_cache"][gen_key] = cache_item

    
cached = st.session_state["report_cache"].get(gen_key)
if cached and (not autoplay_on):
    cyc = cached.get("cycle", int(current_cycle))
    st.download_button(
        "Download report (MD)",
        data=cached["md"],
        file_name=f"battery_report_{selected_bid}_cycle{cyc}.md",
        mime="text/markdown",
        key=f"dl_md_{selected_bid}_{cyc}",
    )
    if cached.get("pdf") is not None:
        st.download_button(
            "Download report (PDF)",
            data=cached["pdf"],
            file_name=f"battery_report_{selected_bid}_cycle{cyc}.pdf",
            mime="application/pdf",
            key=f"dl_pdf_{selected_bid}_{cyc}",
        )
    else:
        st.caption("PDF export unavailable (reportlab not installed).")
elif cached and autoplay_on:
    st.caption("Pause autoplay to download the cached report.")
...
r_ratio 이후 구간으로 이동해 주세요."""
        ...
        if st.session_state["scenarios"]:
            ...
            if not bool(st.session_state.get("auto_play", False)):
                st.download_button(
                    "Download scenarios as CSV",
                    data=csv_bytes,
                    file_name="rul_scenarios.csv",
                    mime="text/csv",
                    key="dl_scenarios_csv",
                )
            else:
                st.caption("Pause autoplay to export scenarios (avoids media churn).")
# =================================================
    ...
    if st_autorefresh is not None:
        ...
        if tick != last_tick:
            ...
            if cur < max_cycle:
                ...
                st.session_state["_auto_play_force_stop"] = True
                if hasattr(st, "rerun"):
                    st.rerun()
                elif hasattr(st, "experimental_rerun"):
                    st.experimental_rerun()
