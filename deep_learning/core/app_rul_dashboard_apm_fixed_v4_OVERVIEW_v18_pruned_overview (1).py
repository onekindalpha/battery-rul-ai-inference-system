...
if st.session_state.get("auto_play", False):
    # streamlit-autorefresh가 있을 때만 자동재생
...
if page == "Overview":
    ...
    # Temperature at discharge start (prefer explicit column name if present)
    temp_measured_0_c_v = _num("temp_measured_0_c")
    if not np.isfinite(temp_measured_0_c_v):
        temp_measured_0_c_v = _num("ambient_temp_c")
    ...
    # -------- layout (v18: pruned overview + optional + advanced) --------
    # Capacity (keep: direct health signals)
    st.markdown("#### Capacity")
    cap1, cap2, cap3 = st.columns(3, gap="small")
    with cap1:
        _metric_card("capacity_ahr<br>(end-of-discharge capacity)", f"{capacity_ahr_v:.3f} Ah" if np.isfinite(capacity_ahr_v) else "—")
    with cap2:
        _metric_card("soh<br>(capacity ratio)", f"{soh_v:.3f}" if np.isfinite(soh_v) else "—")
    with cap3:
    ...
    # Load/Voltage (keep: class + representative C-rate + min voltage)
    st.markdown("#### Load/Voltage")
    lv0, lv1, lv2 = st.columns(3, gap="small")
    with lv0:
        _metric_card("discharge_current_class<br>(1A/2A/4A)", f"{discharge_current_class_v}")
    with lv1:
        _metric_card("eff_c_rate_C<br>(effective mean C-rate)", f"{eff_c_rate_C_v:.3f} C" if np.isfinite(eff_c_rate_C_v) else "—")
    with lv2:
        _metric_card("voltage_min", f"{voltage_min_v:.3f} V" if np.isfinite(voltage_min_v) else "—")

    # Temperature (keep: condition + start temp + rise)
    st.markdown("#### Temperature")
    t0, t1, t2 = st.columns(3, gap="small")
    with t0:
        _metric_card("실험조건 (실온/4도)", exp_cond_v)
    with t1:
        _metric_card("temp_measured_0_c<br>(Temperature_measured[0] at discharge start)", f"{temp_measured_0_c_v:.2f} °C" if np.isfinite(temp_measured_0_c_v) else "—")
    with t2:
        _metric_card("temp_rise_cycle<br>(Tmax - T0)", f"{temp_rise_cycle_v:.2f} °C" if np.isfinite(temp_rise_cycle_v) else "—")

    # Optional diagnostics (toggle)
    show_optional = st.toggle("Show optional diagnostics", value=False, key="toggle_optional_diag_overview")
    if show_optional:
        st.markdown("#### Optional diagnostics")
    ...
    with st.expander("Advanced details", expanded=False):
        st.markdown("##### Load/Voltage details")
        ...
        with a3:
            _metric_card("current_min", f"{current_min_v:.3f} A" if np.isfinite(current_min_v) else "—")
        with a4:
        ...
        with b2:
            _metric_card("eff_current_a<br>(capacity_ahr / (discharge_time_sec/3600))", f"{eff_c_rate_v:.3f} A" if np.isfinite(eff_c_rate_v) else "—")
        with b3:
            _metric_card("c_ref_ahr<br>(battery별 초기 용량 기준값)", f"{c_ref_ahr_v:.3f} Ah" if np.isfinite(c_ref_ahr_v) else "—")
        with b4:
            _metric_card("c_rate_mean<br>(abs(current_mean)/c_ref_ahr)", f"{c_rate_mean_v:.3f} C" if np.isfinite(c_rate_mean_v) else "—")

        c1, c2 = st.columns(2, gap="small")
        with c1:
            _metric_card("dvdt_max_abs<br>(max |dV/dt| during discharge)", f"{dvdt_max_abs_v:.4f} V/s" if np.isfinite(dvdt_max_abs_v) else "—")
        with c2:
            _metric_card("voltage_sensitivity<br>(slope dV/dC, proxy)", f"{lli_v:.4f} V/Ah" if np.isfinite(lli_v) else "—")

        st.markdown("##### Temperature dynamics")
        d1, d2, d3 = st.columns(3, gap="small")
        with d1:
            _metric_card("temperature_measured_max", f"{temp_max_v:.2f} °C" if np.isfinite(temp_max_v) else "—")
        with d2:
            _metric_card("dTdt_max<br>(max dT/dt during discharge)", f"{dTdt_max_v:.4f} °C/s" if np.isfinite(dTdt_max_v) else "—")
        with d3:
            _metric_card("thermal_stress<br>(exp(alpha * (Tmax - Tref)))", f"{thermal_stress_v:.3f} (unitless)" if np.isfinite(thermal_stress_v) else "—")
        ...
        with e2:
            _metric_card("dcr_growth<br>(log ratio vs early life)", f"{dcr_growth_v:.4f}" if np.isfinite(dcr_growth_v) else "—")
        with e3:
            _metric_card("resistance_sensitivity<br>(slope dRe/dC, proxy)", f"{lam_v:.4f} Ω/Ah" if np.isfinite(lam_v) else "—")
