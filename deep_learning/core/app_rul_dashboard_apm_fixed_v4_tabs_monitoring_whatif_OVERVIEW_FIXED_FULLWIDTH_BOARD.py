...
with tab_overview:
    ...
    with c1:
        _metric_card("capacity_ahr<br>(end-of-discharge capacity, Ah)", f"{capacity_ahr_v:.3f}" if np.isfinite(capacity_ahr_v) else "—")
    with c2:
        _metric_card("soh<br>(capacity(% initial))", f"{soh_v:.3f}" if np.isfinite(soh_v) else "—")
    with c3:
        _metric_card("capacity_derivative<br>= diff(C)", f"{capacity_derivative_v:.4f}" if np.isfinite(capacity_derivative_v) else "—")
    with c4:
        _metric_card("lli<br>= slope(dC, dV)<br>/ Voltage sensitivity to capacity loss (proxy)", f"{lli_v:.4f}" if np.isfinite(lli_v) else "—")
    ...
    with l6:
        _metric_card("discharge_current_class", discharge_current_class_v if discharge_current_class_v is not None else "—")

    l7, l8, l9, l10, l11, l12 = st.columns(6, gap="small")
    with l7:
        _metric_card("eff_c_rate<br>(capacity_ahr / (discharge_time_sec/3600))", (f"{eff_c_rate_v:.3f} A" if np.isfinite(eff_c_rate_v) else "—"))
    with l8:
        _metric_card("c_ref_ahr<br>(battery별 초기 용량 기준값)", f"{c_ref_ahr_v:.3f}" if np.isfinite(c_ref_ahr_v) else "—")
    with l9:
        _metric_card("current_min", f"{current_min_v:.3f}" if np.isfinite(current_min_v) else "—")
    with l10:
        _metric_card("c_rate_peak<br>= abs(current_min) / c_ref_ahr", f"{c_rate_peak_v:.3f}" if np.isfinite(c_rate_peak_v) else "—")
    with l11:
        _metric_card("c_rate_mean<br>= abs(current_mean) / c_ref_ahr", f"{c_rate_mean_v:.3f}" if np.isfinite(c_rate_mean_v) else "—")
    with l12:
        _metric_card("eff_c_rate_C<br>= eff_c_rate / c_ref_ahr", f"{eff_c_rate_C_v:.3f}" if np.isfinite(eff_c_rate_C_v) else "—")
    ...
    with t2:
        _metric_card("(Temperature_measured[0] at<br>discharge start, °C)", (f"{ambient_temp_c_v:.2f}°C" if np.isfinite(ambient_temp_c_v) else "—"))
    with t3:
    ...
    with t5:
        _metric_card("temp_rise_cycle<br>(Tmax - Tstart)", (f"{temp_rise_cycle_v:.2f}°C" if np.isfinite(temp_rise_cycle_v) else "—"))
    with t6:
        _metric_card("thermal_stress<br>= exp(alpha*(Tmax - Tref))", (f"{thermal_stress_v:.3f}" if np.isfinite(thermal_stress_v) else "—"))
    ...
    with r3:
        _metric_card("lam<br>= slope(dC, dRe)<br>/ Resistance sensitivity to capacity loss (proxy)", f"{lam_v:.4f}" if np.isfinite(lam_v) else "—")
