...
def load_cycle_features():
    ...
    # Normalize columns across different cycle-feature tables
    if 'battery' in df.columns and 'battery_id' not in df.columns:
        df['battery_id'] = df['battery'].astype(str)
    if 'cycle_num' in df.columns and 'cycle' not in df.columns:
        df['cycle'] = pd.to_numeric(df['cycle_num'], errors='coerce').fillna(-1).astype(int)
    if 'cycle' in df.columns:
        df['cycle'] = pd.to_numeric(df['cycle'], errors='coerce').fillna(-1).astype(int)
    if 'cap_pct' not in df.columns and 'soh' in df.columns:
        df['cap_pct'] = pd.to_numeric(df['soh'], errors='coerce') * 100.0
...
# -----------------------------
# Page selector (runs ONLY selected page to avoid rerun freeze)
# -----------------------------
page = st.radio(
    "page",
