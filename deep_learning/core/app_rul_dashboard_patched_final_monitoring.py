...
if cycle_ctx.row is None:
    ...
    with r1c2:
        if not math.isnan(cap_pct):
            ...
            st.metric("Capacity (vs. initial)", "N/A")
    with r1c3:
        st.metric("DCR", f"{dcr_v:.4f} Ω" if not math.isnan(dcr_v) else "N/A")

    r2c1, r2c2, r2c3 = st.columns(3)
    with r2c1:
        st.metric("Impedance sum", f"{imp_v:.4f} Ω" if not math.isnan(imp_v) else "N/A")
    with r2c2:
...
with st.expander("🆚 Compare batteries (Geotab-style)", expanded=False):
    ...
    if df_feat is None:
        ...
        show_band = st.checkbox("전체 배터리 분포(중앙값/사분위) 함께 보기", value=True)

        fig_cmp = go.Figure()

        # fleet band (median + IQR) on cycle index
        if show_band and metric_key in df_feat.columns:
            tmp = df_feat[["battery_id", "cycle", metric_key]].dropna()
            if not tmp.empty:
