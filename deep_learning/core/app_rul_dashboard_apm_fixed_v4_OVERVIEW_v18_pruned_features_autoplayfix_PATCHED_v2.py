...
# 현재 run에서 사용할 실제 cycle 값 결정 (auto_play는 "읽기만" 하고, 증가는 아래 Auto-play 루프에서 처리)
if st.session_state.get("auto_play", False):
    current_cycle = int(st.session_state.get("play_cycle", current_cycle_slider))
else:
...
if page == "Overview":
    # -------------------------------------------------
    # Forecast diagnostics (Predicted EOL / residual / calibration) — Overview
    # -------------------------------------------------
    pred_tl = None
    if fut_cycles.size > 0 and fut_rul_pred.size > 0 and fut_rul_true.size == fut_rul_pred.size:
        m_valid = np.isfinite(fut_cycles) & np.isfinite(fut_rul_pred) & np.isfinite(fut_rul_true)
        if fut_rul_std.size == fut_rul_pred.size:
            m_valid = m_valid & np.isfinite(fut_rul_std) & (fut_rul_std > 0)

        x_pred_all = fut_cycles[m_valid].astype(float)
        y_pred_rul_all = fut_rul_pred[m_valid].astype(float)
        y_true_rul_all = fut_rul_true[m_valid].astype(float)
        y_std_all = fut_rul_std[m_valid].astype(float) if fut_rul_std.size == fut_rul_pred.size else np.full_like(y_pred_rul_all, np.nan)

        y_pred_eol_all = x_pred_all + y_pred_rul_all
        resid_rul_all = y_pred_rul_all - y_true_rul_all

        z_all = np.full_like(resid_rul_all, np.nan, dtype=float)
        mz = np.isfinite(resid_rul_all) & np.isfinite(y_std_all) & (y_std_all > 0)
        z_all[mz] = resid_rul_all[mz] / y_std_all[mz]

        pred_tl = {
            "x": x_pred_all,
            "pred_rul": y_pred_rul_all,
            "true_rul": y_true_rul_all,
            "std": y_std_all,
            "pred_eol": y_pred_eol_all,
            "resid_rul": resid_rul_all,
            "z": z_all,
        }

    def _recent_idx(x: np.ndarray, current_cyc: float, n: int = 200) -> np.ndarray:
        if x is None or x.size == 0:
            return np.asarray([], dtype=int)
        idx = np.where(np.isfinite(x) & (x <= float(current_cyc)))[0]
        if idx.size == 0:
            return np.asarray([], dtype=int)
        return idx[-int(n):]

    # Coverage (calibration) + alert signals (computed up to current_cycle)
    cov_1s = float("nan")
    cov_2s = float("nan")
    z_cur = float("nan")
    fast_deg_active = False
    outlier_now = False
    outlier_count = 0
    onset_msg = None
    onset_cycle = None

    if pred_tl is not None and pred_tl["x"].size > 0:
        # recent window coverage (last 200 points up to cursor)
        ridx = _recent_idx(pred_tl["x"], float(current_cycle), n=200)
        if ridx.size > 0:
            resid = pred_tl["resid_rul"][ridx]
            stdv = pred_tl["std"][ridx]
            z = pred_tl["z"][ridx]

            ok1 = np.isfinite(resid) & np.isfinite(stdv) & (stdv > 0) & (np.abs(resid) <= 1.0 * stdv)
            ok2 = np.isfinite(resid) & np.isfinite(stdv) & (stdv > 0) & (np.abs(resid) <= 2.0 * stdv)
            cov_1s = float(np.mean(ok1)) if ok1.size > 0 else float("nan")
            cov_2s = float(np.mean(ok2)) if ok2.size > 0 else float("nan")

            # z at cursor (closest predicted point)
            try:
                ic = int(np.argmin(np.abs(pred_tl["x"] - float(current_cycle))))
                z_cur = float(pred_tl["z"][ic]) if np.isfinite(pred_tl["z"][ic]) else float("nan")
            except Exception:
                z_cur = float("nan")

            # Alerts
            outlier_now = bool(np.isfinite(z_cur) and abs(z_cur) > 3.0)
            outlier_count = int(np.sum(np.isfinite(z) & (np.abs(z) > 3.0)))

            # Fast degradation: z < -2 for 5 consecutive (ending at current)
            consec_neg = 0
            for v in z[::-1]:
                if np.isfinite(v) and (v < -2.0):
                    consec_neg += 1
                else:
                    break
            fast_deg_active = consec_neg >= 5

        # Onset detection (sustained deviation) on series up to cursor
        idx_upto = np.where(np.isfinite(pred_tl["x"]) & (pred_tl["x"] <= float(current_cycle)))[0]
        if idx_upto.size > 0:
            x_u = pred_tl["x"][idx_upto]
            z_u = pred_tl["z"][idx_upto]

            # Method A: rolling median drift (baseline=0)
            try:
                z_med = pd.Series(z_u).rolling(window=9, min_periods=5).median().to_numpy()
            except Exception:
                z_med = z_u

            onset_a = find_onset(z_med, thresh=2.0, min_run=8, direction="abs")
            # Method B: |resid| > k*sigma for N consecutive (here, z-threshold)
            onset_b_neg = find_onset(z_u, thresh=2.0, min_run=8, direction="neg")
            onset_b_pos = find_onset(z_u, thresh=2.0, min_run=8, direction="pos")

            # Prefer: negative(열화 가속/이벤트) > positive(낙관 편향) > abs drift
            onset_i = onset_b_neg if onset_b_neg is not None else (onset_b_pos if onset_b_pos is not None else onset_a)

            if onset_i is not None:
                onset_cycle = int(x_u[int(onset_i)])
                if onset_b_neg is not None:
                    onset_msg = f"이상 징후 시작 추정: cycle {onset_cycle} (연속 8윈도우에서 -2σ 이하)"
                elif onset_b_pos is not None:
                    onset_msg = f"이상 징후 시작 추정: cycle {onset_cycle} (연속 8윈도우에서 +2σ 이상)"
                else:
                    onset_msg = f"이상 징후 시작 추정: cycle {onset_cycle} (잔차 이동중앙값이 기준선에서 이탈 후 지속)"
            else:
                onset_msg = "이상 징후 시작 추정: N/A (지속적인 잔차 이탈을 찾지 못함)"
    ...
    with left:
        ...
        if not math.isnan(cur_pi):
            ...
            st.caption("Prediction confidence: N/A (not in prediction region)")
                        ...
                        # --- Calibration / alert snapshot (recent 200 points in prediction region) ---
        pred_eol_cur = float("nan")
        if not math.isnan(current_pred_rul):
            pred_eol_cur = float(current_cycle) + float(current_pred_rul)

        k4, k5, k6 = st.columns(3)
        with k4:
            st.metric("Pred EOL (cycle)", f"{pred_eol_cur:.0f}" if np.isfinite(pred_eol_cur) else "N/A")
        with k5:
            st.metric("Coverage ±1σ (last200)", f"{100.0*cov_1s:.1f}%" if np.isfinite(cov_1s) else "N/A")
        with k6:
            st.metric("Coverage ±2σ (last200)", f"{100.0*cov_2s:.1f}%" if np.isfinite(cov_2s) else "N/A")

        a1, a2, a3 = st.columns(3)
        with a1:
            st.metric("Fast-deg alert", "ON" if fast_deg_active else "OFF")
        with a2:
            st.metric("|z|>3 outliers", f"{outlier_count}" if outlier_count is not None else "N/A")
        with a3:
            st.metric("z@current", f"{z_cur:.2f}" if np.isfinite(z_cur) else "N/A")
...
# Overview signals are rendered below the RUL trajectory (full-width board)
    with right:
        ...
        if in_future_region and fut_cycles.size > 0 and fut_rul_pred.size > 0:
            ...
            if fut_x.size > 0:
                                # Uncertainty band (±2σ) + (±1σ)
                if fut_std_y.size == fut_pred_y.size and not np.all(np.isnan(fut_std_y)):
                    upper2 = fut_pred_y + 2.0 * fut_std_y
                    lower2 = np.maximum(0.0, fut_pred_y - 2.0 * fut_std_y)
                    upper1 = fut_pred_y + 1.0 * fut_std_y
                    lower1 = np.maximum(0.0, fut_pred_y - 1.0 * fut_std_y)

                    # Outer band (±2σ)
                    fig.add_trace(
                        go.Scatter(
                            x=np.concatenate([fut_x, fut_x[::-1]]),
                            y=np.concatenate([upper2, lower2[::-1]]),
                            fill="toself",
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=np.concatenate([fut_x, fut_x[::-1]]),
                            y=np.concatenate([upper1, lower1[::-1]]),
                            fill="toself",
    ...
    # -------------------------------------------------
    # Forecast diagnostics (Overview): Predicted EOL, ΔPred EOL, residual timeline, change-point, calibration
    # -------------------------------------------------
    st.markdown("### Forecast diagnostics (EOL / residual / calibration)")

    if pred_tl is None or pred_tl["x"].size == 0:
        st.info("Forecast diagnostics는 예측 구간(r_ratio 이후)에서만 표시됩니다. (현재 커서가 예측 구간 밖이면 N/A)")
    else:
        dc1, dc2 = st.columns(2, gap="large")

        # ---- (1) Predicted EOL trajectory + ΔPred_EOL ----
        with dc1:
            x = pred_tl["x"]
            eol = pred_tl["pred_eol"]
            stdv = pred_tl["std"]

            fig_eol = make_subplots(specs=[[{"secondary_y": True}]])
            # Uncertainty bands (EOL): σ_EOL = σ_RUL
            if stdv.size == eol.size and not np.all(np.isnan(stdv)):
                eol_u2 = eol + 2.0 * stdv
                eol_l2 = eol - 2.0 * stdv
                eol_u1 = eol + 1.0 * stdv
                eol_l1 = eol - 1.0 * stdv
                ...
                fig_eol.add_trace(
                    go.Scatter(
                        ...
                        y=np.concatenate([eol_u2, eol_l2[::-1]]),
                        fill="toself",
                ...
                fig_eol.add_trace(
                    go.Scatter(
                        x=np.concatenate([x, x[::-1]]),
                        y=np.concatenate([eol_u1, eol_l1[::-1]]),
                        fill="toself",
            ...
            fig_eol.add_trace(
                go.Scatter(
                    ...
                    y=eol,
                    mode="lines",
                    name="Predicted EOL (cycle)",
                    line=dict(width=2),
                ),
                secondary_y=False,
            )

            # ΔPred_EOL (aux): where forecast shifted (improve/worsen)
            if eol.size >= 2:
                deol = np.diff(eol)
                x_deol = x[1:]
                fig_eol.add_trace(
                    go.Scatter(
                        ...
                        y=deol,
                        mode="lines",
                        name="ΔPred_EOL (t - t-1)",
                        line=dict(width=1),
                        opacity=0.8,
                    ),
                    secondary_y=True,
                )
                # y=0 line on secondary axis
                fig_eol.add_shape(
                    type="line",
                    x0=float(np.nanmin(x_deol)),
                    x1=float(np.nanmax(x_deol)),
                    y0=0.0,
                    y1=0.0,
                    xref="x",
                    yref="y2",
            ...
            # markers
            fig_eol.add_vline(x=float(current_cycle), line_dash="dash", line_color="red")
            if onset_cycle is not None:
                fig_eol.add_vline(x=float(onset_cycle), line_dash="dot", line_color="purple")
            ...
            fig_eol.update_xaxes(title_text="Cycle")
            fig_eol.update_yaxes(title_text="Predicted EOL (cycle)", secondary_y=False)
            fig_eol.update_yaxes(title_text="ΔPred_EOL (cycle/cycle)", secondary_y=True)

            st.plotly_chart(fig_eol, use_container_width=True, key="overview_pred_eol")

        # ---- (2) Residual + z-score + change-point ----
        with dc2:
            x = pred_tl["x"]
            resid = pred_tl["resid_rul"]  # resid_RUL = pred - true
            z = pred_tl["z"]
            ...
            fig_res.add_trace(
                go.Scatter(
                    ...
                    y=resid,
                    mode="lines",
                    name="resid_RUL = pred - true",
                    line=dict(width=2),
                ),
                secondary_y=False,
            )
            # baseline 0 line
            fig_res.add_hline(y=0.0, line_dash="dash", line_color="rgba(0,0,0,0.35)")
            ...
            # thresholds on secondary axis (±2, ±3)
            for thr in [2.0, 3.0]:
                fig_res.add_shape(
                    type="line",
            ...
            fig_res.add_vline(x=float(current_cycle), line_dash="dash", line_color="red")
            if onset_cycle is not None:
                fig_res.add_vline(x=float(onset_cycle), line_dash="dot", line_color="purple")
            ...
            fig_res.update_xaxes(title_text="Cycle")
            fig_res.update_yaxes(title_text="resid_RUL (cycles)", secondary_y=False)
            fig_res.update_yaxes(title_text="z-score", secondary_y=True)

            st.plotly_chart(fig_res, use_container_width=True, key="overview_residual")

            st.caption(
                "resid_RUL(t) = pred_RUL(t) - true_RUL(t). "
                "resid_RUL이 **음수로 급락**하면 실제보다 더 빨리 끝난다고 보기 시작한 것(열화 가속/이벤트/센서 문제 후보), "
                "resid_RUL이 **양수로 증가**하면 실제보다 낙관적으로 보는 것(모델 미스/새 운영조건/캘리브레이션 문제 후보)."
            )

        # ---- Summary line (onset + calibration + alerts) ----
        if onset_msg is not None:
            st.info(onset_msg)

        # Coverage / Alerts snapshot (recent 200 points)
        ...
        with cC:
            st.metric("z@current", f"{z_cur:.2f}" if np.isfinite(z_cur) else "N/A")
        with cD:
            st.metric("Outliers (|z|>3, last200)", f"{outlier_count}" if outlier_count is not None else "N/A")

        alert_txt = []
        if fast_deg_active:
            alert_txt.append("🚨 급격 열화 의심 (z < -2가 5회 연속)")
        if outlier_now:
            alert_txt.append("⚠️ 센서/입력 이상 가능 (|z| > 3 단발)")
        if not alert_txt:
            alert_txt.append("✅ Alert 없음 (현재 기준)")

        st.markdown("**Alert status:** " + " · ".join(alert_txt))
...
# ----- Signals board (below the chart, full width) -----
    ...
    temp_measured_0_c_v = _num("temp_measured_0_c")
    if not np.isfinite(temp_measured_0_c_v):
        temp_measured_0_c_v = _num("ambient_temp_c")  # fallback (= Temperature_measured[0] at discharge start, °C)
    ...
    if not np.isfinite(temp_rise_cycle_v) and np.isfinite(temp_max_v) and np.isfinite(temp_measured_0_c_v):
        temp_rise_cycle_v = temp_max_v - temp_measured_0_c_v

    # experiment condition fallback (실온/4도)
    if exp_cond_v in ["", "nan", "None"]:
        if np.isfinite(temp_measured_0_c_v):
            exp_cond_v = "저온 4°C" if temp_measured_0_c_v <= 10.0 else "실온"
        else:
            exp_cond_v = "—"
    ...
    if not np.isfinite(thermal_stress_v) and np.isfinite(temp_max_v):
        try:
            ...
            pass    # -------- layout (pruned Overview signals) --------
    st.markdown("#### Capacity/Health")
    cap1, cap2, cap3 = st.columns(3, gap="small")
    with cap1:
        _metric_card("capacity_ahr<br>(Ah)", f"{capacity_ahr_v:.3f} Ah" if np.isfinite(capacity_ahr_v) else "—")
    with cap2:
        _metric_card("soh<br>(ratio)", f"{soh_v:.3f}" if np.isfinite(soh_v) else "—")
    with cap3:
        _metric_card("capacity_derivative<br>(ΔC per cycle)", f"{capacity_derivative_v:.4f} Ah" if np.isfinite(capacity_derivative_v) else "—")

    st.markdown("#### Load/Voltage")
    lv1, lv2, lv3 = st.columns(3, gap="small")
    with lv1:
        _metric_card("discharge_current_class<br>(1A/2A/4A)", f"{discharge_current_class_v}")
    with lv2:
        _metric_card("eff_c_rate_C<br>(effective C-rate)", f"{eff_c_rate_C_v:.3f} C" if np.isfinite(eff_c_rate_C_v) else "—")
    with lv3:
        _metric_card("voltage_min<br>(V)", f"{voltage_min_v:.3f} V" if np.isfinite(voltage_min_v) else "—")

    st.markdown("#### Temperature")
    t1, t2 = st.columns(2, gap="small")
    with t1:
        _metric_card("temp_measured_0_c<br>(°C)", f"{temp_measured_0_c_v:.2f} °C" if np.isfinite(temp_measured_0_c_v) else "—")
    with t2:
        _metric_card("temp_rise_cycle<br>(°C)", f"{temp_rise_cycle_v:.2f} °C" if np.isfinite(temp_rise_cycle_v) else "—")

    # Optional diagnostics (+2)
    show_opt = st.toggle("Show diagnostics (+2)", value=False, key="overview_show_opt")
    if show_opt:
        o1, o2 = st.columns(2, gap="small")
        with o1:
            _metric_card("dcr_growth<br>(log ratio vs early life)", f"{dcr_growth_v:.4f}" if np.isfinite(dcr_growth_v) else "—")
        with o2:
            _metric_card("c_rate_peak<br>(abs(current_min)/c_ref_ahr)", f"{c_rate_peak_v:.3f} C" if np.isfinite(c_rate_peak_v) else "—")

    # Keep the rest accessible without cluttering overview
    with st.expander("Advanced details (optional)", expanded=False):
        ...
        with a4:
            _metric_card("eff_c_rate<br>(A)", f"{eff_c_rate_v:.3f} A" if np.isfinite(eff_c_rate_v) else "—")

        b1, b2, b3, b4 = st.columns(4, gap="small")
        with b1:
            _metric_card("discharge_time_sec", f"{discharge_time_sec_v:.0f} s" if np.isfinite(discharge_time_sec_v) else "—")
        with b2:
            _metric_card("c_ref_ahr", f"{c_ref_ahr_v:.3f} Ah" if np.isfinite(c_ref_ahr_v) else "—")
        with b3:
            _metric_card("dvdt_max_abs", f"{dvdt_max_abs_v:.4f} V/s" if np.isfinite(dvdt_max_abs_v) else "—")
        with b4:
            _metric_card("dTdt_max", f"{dTdt_max_v:.4f} °C/s" if np.isfinite(dTdt_max_v) else "—")

        c1, c2, c3 = st.columns(3, gap="small")
        with c1:
            _metric_card("temperature_measured_max", f"{temp_max_v:.2f} °C" if np.isfinite(temp_max_v) else "—")
        with c2:
            _metric_card("thermal_stress<br>(unitless)", f"{thermal_stress_v:.3f}" if np.isfinite(thermal_stress_v) else "—")
        with c3:
            _metric_card("dcr(Re)", f"{dcr_v:.4f} Ω" if np.isfinite(dcr_v) else "—")

        d1, d2 = st.columns(2, gap="small")
        with d1:
            _metric_card("lli (proxy)", f"{lli_v:.4f}" if np.isfinite(lli_v) else "—")
        with d2:
            _metric_card("lam (proxy)", f"{lam_v:.4f}" if np.isfinite(lam_v) else "—")
...
# =================================================
# Auto-play loop (non-blocking preferred)
# - Updates play_cycle ONLY here (not in the sidebar logic)
# - Stops automatically at max_cycle
# =================================================
AUTO_PLAY_DELAY_SEC = 0.6
AUTO_PLAY_STEP = 1
...
if st.session_state.get("auto_play", False):
    ...
    if st_autorefresh is not None:
        ...
        # NOTE:
        # Without a timer component, Streamlit cannot advance UI state periodically
        # without blocking the server thread (time.sleep) + forcing reruns, which
        # commonly causes "white flash" / flicker and breaks autoplay.
        #
        # So we *disable* autoplay and tell the user how to enable smooth autoplay.
        st.sidebar.warning(
            "자동 재생(타이머)이 설치되어 있지 않아 자동 재생을 비활성화했습니다.\n"
            "원활한 자동 재생을 원하면 `pip install streamlit-autorefresh` 후 다시 실행하세요."
        )
        st.session_state["auto_play"] = False
