...
# ---- lightweight run-time profiling (for autoplay pacing) ----
...
# Autoplay bookkeeping (timer key epoch + last tick)
if "autoplay_epoch" not in st.session_state:
    st.session_state["autoplay_epoch"] = 0
if "_prev_auto_play" not in st.session_state:
    st.session_state["_prev_auto_play"] = bool(auto_play)
if "_last_autoplay_tick" not in st.session_state:
    st.session_state["_last_autoplay_tick"] = None

# 토글 변화 감지 → key 변경(프론트 타이머 잔존/중복 방지)
if bool(st.session_state.get("_prev_auto_play")) != bool(auto_play):
    st.session_state["_prev_auto_play"] = bool(auto_play)
    st.session_state["autoplay_epoch"] = int(st.session_state.get("autoplay_epoch", 0)) + 1
    st.session_state["_last_autoplay_tick"] = None

def _compute_autoplay_interval_ms() -> int:
    """직전 실행 시간 기반으로 refresh 주기를 늘려 overlap(먹통/재연결)을 방지."""
    last_ms = st.session_state.get("_last_run_ms")
    if last_ms is None:
        base = 5000
    else:
        try:
            ...
            last_ms = 0
        # 1.8x headroom + safety buffer
        base = int(last_ms * 1.8 + 400)
        if base < 2500:
            base = 2500
    if base > 15000:
        base = 15000
...
# Autoplay step (슬라이더 렌더링 전에 play_cycle을 먼저 갱신 → UI 값도 따라오게)
if auto_play:
    if int(st.session_state["play_cycle"]) >= max_cycle:
        # 끝에 도달하면 refresh를 더 걸지 않고 고정 (체감상 "계속 뿌얘짐" 방지)
        st.sidebar.info("최대 사이클에 도달했습니다. 자동 재생을 끄면 수동 조절이 가능합니다.")
    elif st_autorefresh is not None:
        interval_ms = _compute_autoplay_interval_ms()
        tick = st_autorefresh(
            interval=interval_ms,
            key=f"autoplay_refresh_{st.session_state['autoplay_epoch']}",
        )
        # tick이 변한 경우에만 1 step 진행 (다른 위젯 rerun에 의해 중복 증가 방지)
        if st.session_state.get("_last_autoplay_tick") != tick:
            st.session_state["_last_autoplay_tick"] = tick
            st.session_state["play_cycle"] = min(max_cycle, int(st.session_state["play_cycle"]) + 1)
    else:
        # sleep+rerun fallback은 UI를 "먹통/재연결"처럼 보이게 하므로 차단
        st.sidebar.warning(
            "자동 재생은 'streamlit-autorefresh' 설치가 필요합니다. "
            "설치 후 다시 실행해 주세요: pip install streamlit-autorefresh"
        )
        # autoplay는 켜져 있어도 동작은 안 하게 두고(사용자가 해제 가능), 수동 조절은 유지합니다.

    # disabled slider 표시값을 현재 play_cycle로 동기화
    st.session_state["current_cycle_slider"] = int(st.session_state["play_cycle"])
...
current_cycle_slider = st.sidebar.slider(
    ...
    max_value=max_cycle,
    step=1,
    value=int(st.session_state.get("current_cycle_slider", st.session_state.get("play_cycle", max(min_cycle, 1)))),
    key="current_cycle_slider",
    disabled=bool(auto_play),
)
