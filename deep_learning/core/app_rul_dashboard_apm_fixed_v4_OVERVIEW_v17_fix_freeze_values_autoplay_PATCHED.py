...
AUTOPLAY_PAUSE_AFTER_UI_S = 1.2

if st.session_state.get("auto_play", False):
    # Advance ONLY on actual autorefresh ticks (not on every rerun caused by widget interaction)
    if (
        st_autorefresh is not None
        and int(st.session_state.get("play_cycle", min_cycle)) < max_cycle
        and (not _recent_user_action(AUTOPLAY_PAUSE_AFTER_UI_S))
        and _page_now in ("Overview", "Anomaly")
    ):
