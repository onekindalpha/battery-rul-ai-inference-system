...
with tab_overview:
    ...
    with l5:
        _metric_card("discharge_time_sec", f"{discharge_time_sec_v:.0f}" if np.isfinite(discharge_time_sec_v) else "—")
    with l6:
        _metric_card("discharge_current_class (1A/2A/4A)", discharge_current_class_v if discharge_current_class_v is not None else "—")
    ...
    with l10:
        _metric_card("c_rate_mean = abs(current_mean) / c_ref_ahr", f"{c_rate_mean_v:.3f}" if np.isfinite(c_rate_mean_v) else "—")
    with l11:
        _metric_card("eff_c_rate_C = eff_c_rate / c_ref_ahr", f"{eff_c_rate_C_v:.3f}" if np.isfinite(eff_c_rate_C_v) else "—")
    ...
    with t2:
        _metric_card("(Temperature_measured[0] at discharge start, °C)", (f"{ambient_temp_c_v:.2f}°C" if np.isfinite(ambient_temp_c_v) else "—"))
    with t3:
