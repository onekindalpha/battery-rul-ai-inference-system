...
SCENARIO_FEATURES = {
    "soh": {
        "label": "State of Health, SoH (0–1)",
        "fallback_min": 0.6,
        "fallback_max": 1.0,
        "step": 0.01,
    },
    "capacity_mean": {
        "label": "Mean discharge capacity (Ah)",
        "fallback_min": 1.0,
        "fallback_max": 3.5,
        "step": 0.05,
    },
    ...
    "current_min": {
        "label": "Minimum current (A, most negative during discharge)",
        "fallback_min": -10.0,
        "fallback_max": 0.0,
        "step": 0.2,
    },
    "dcr": {
        "label": "DC resistance, DCR (Ω)",
        "fallback_min": 0.0,
        "fallback_max": 1.0,
        "step": 0.001,
    },
    ...
    "ambient_temp_c": {
        "label": "Cell temperature at discharge start (°C)",
        "fallback_min": 10.0,
        "fallback_max": 45.0,
        "step": 1.0,
    },
    "temp_rise_cycle": {
        "label": "Cell temperature rise per cycle, ΔT (°C)",
        "fallback_min": 0.0,
        "fallback_max": 20.0,
        "step": 0.5,
    },
...
def load_cycle_features():
    ...
    # battery-level protocol ambient cohort (from README groups)
    df["ambient_protocol_group"] = df["battery_id"].astype(str).apply(protocol_ambient_group)
    return df
...
# -------------------------------------------------
# Protocol ambient cohort (battery-level; derived from NASA README groups)
# NOTE:
#   In our preprocessing, `ambient_temp_c` was computed from the *first sample* of
#   `Temperature_measured` during discharge (i.e., a cell temperature proxy),
#   so it can vary within a single battery and should NOT be used as a cohort key.
# -------------------------------------------------
def protocol_ambient_group(bid: str) -> str:
    bid = str(bid)
    try:
        n = int(re.sub(r"[^0-9]", "", bid))
    except Exception:
        return "unknown"

    # Battery groups described in the NASA dataset README files
    # 25–28: 24°C, 29–32: 43°C, 33–36: 24°C
    # 38–40: mixed 24/44°C, 41–44: 4°C, 45–48: 4°C, 49–52: 4°C, 53–56: 4°C
    if 25 <= n <= 28:
        return "ambient_24c"
    if 29 <= n <= 32:
        return "ambient_43c"
...
COHORT_OPTIONS = [
    ("All cells", "all"),
    ("Ambient 4°C (protocol)", "ambient_4c"),
    ("Ambient 24°C (protocol)", "ambient_24c"),
    ("Ambient 43°C (protocol)", "ambient_43c"),
    ("Mixed 24/44°C (protocol)", "ambient_mixed_24_44c"),
    ("Unknown ambient (protocol)", "unknown"),
]
...
def apply_cohort_filter(df: pd.DataFrame, cohort_key: str) -> pd.DataFrame:
    if cohort_key == "all":
        return df
    if "ambient_protocol_group" not in df.columns:
        return df
    return df[df["ambient_protocol_group"] == cohort_key]
...
show_demo_true = st.sidebar.checkbox("Show ground-truth RUL (demo only)", value=False, key="show_demo_true")
...
# ❗ 이 문구 유지
st.sidebar.caption(
    "Tip: For short-life cells (e.g., B0042/B0043), moving r_ratio earlier (e.g., 0.25) helps you see predictions right after the adaptation window."
)
...
with top_left:
    ...
    status_basis = "예측 기반" if (initial_rul > 0 and not math.isnan(current_pred_rul)) else "예측 불가"
    rul_for_status = current_pred_rul if (initial_rul > 0 and not math.isnan(current_pred_rul)) else float("nan")
...
# 🔹 3개 카드: 현재 사이클 / 예측 RUL / 상태(작은 카드)
    h1, h3, h4 = st.columns(3)
...
# 🔹 상태 GIF (옵션): 화면 공간 절약을 위해 기본 OFF
    show_status_gif = st.sidebar.checkbox("상태 GIF 표시", value=False, help="설명용 예시 이미지입니다. 공간이 불편하면 끄세요.")
    if show_status_gif:
        gif_to_show = None
        if not math.isnan(rul_pct):
...
with top_right:
    ...
    # 1) Ground-truth RUL (demo only)
    if st.session_state.get("show_demo_true", False):
        fig.add_trace(
            go.Scatter(
                x=cycles,
                y=rul_true,
    ...
    # '현재까지 예측구간' = future region(=split 이후) 중에서, 현재 cycle까지 누적된 backtest 오차
    rmse_dyn = mae_dyn = None
    if fut_cycles.size > 0 and fut_rul_pred.size == fut_cycles.size:
        mask_dyn = (~np.isnan(fut_rul_pred)) & (fut_cycles <= float(current_cycle))
        if np.any(mask_dyn):
            diff_dyn = fut_rul_pred[mask_dyn] - fut_rul_true[mask_dyn]
...
# =================================================
with st.expander("📈 Monitoring (selected battery)", expanded=True):
    df_feat = load_cycle_features()
    if df_feat is None:
        ...
        if cohort_key_mon != "all":
            st.info(
                "Note: cohorts are defined by *protocol ambient setpoint* (battery-level). "
                "This avoids the common confusion where a per-cycle cell temperature proxy looks like ambient changing within a battery."
            )
        ...
        simplified_mon = st.checkbox(
            "Simplified view (recommended)",
            value=True,
            help="Show one metric per plot to reduce visual clutter.",
            key="mon_simplified",
        )
)
        ...
        if df_b.empty:
            ...
            if simplified_mon:
                metric_choices = [
                    ...
                    ('SoH', 'soh'),
                    ('DCR', 'dcr'),
                    ('Impedance sum', 'impedance_sum'),
                    ('Cell temperature (mean)', 'temperature_mean'),
                    ('Cell temperature (max)', 'temperature_measured_max'),
                    ('Thermal stress', 'thermal_stress'),
                ]
                default_metrics = ['capacity_pct', 'dcr', 'impedance_sum', 'temperature_mean', 'thermal_stress']
                sel = st.multiselect('Metrics to display', [m[0] for m in metric_choices],
                                  default=[m[0] for m in metric_choices if m[1] in default_metrics])
                key_map = {m[0]: m[1] for m in metric_choices}

                # Prepare capacity % series (if available)
                cap_pct_series = None
                if 'capacity_mean' in df_b.columns:
                    try:
                        cap0 = float(df_b.iloc[0]['capacity_mean'])
                ...
                for label in sel:
                    ...
                    if fk == 'capacity_pct':
                        if cap_pct_series is None:
                            continue
                        y = cap_pct_series.astype(float).values
                        fig_s.add_trace(go.Scatter(x=x, y=y, mode='lines', showlegend=False))
                        if overlay_band_mon:
                            ...
                            if band is not None:
                                ...
                                if x_band.size > 0:
                                    ...
                                    fig_s.add_trace(go.Scatter(x=x_band, y=y_q75, mode='lines', line=dict(width=0), showlegend=False, hoverinfo='skip'))
                                    fig_s.add_trace(go.Scatter(x=x_band, y=y_q25, mode='lines', fill='tonexty', line=dict(width=0), opacity=0.15, showlegend=False, hoverinfo='skip'))
                                    fig_s.add_trace(go.Scatter(x=x_band, y=y_med, mode='lines', line=dict(dash='dash'), showlegend=False, hoverinfo='skip'))
                        fig_s.update_layout(height=240, margin=dict(l=30, r=10, t=10, b=25), xaxis_title='Cycle', yaxis_title='Capacity %')
                    else:
                        if fk not in df_b.columns:
                            continue
                        y = df_b[fk].astype(float).values
                        fig_s.add_trace(go.Scatter(x=x, y=y, mode='lines', showlegend=False))
                        if overlay_band_mon:
                            band = compute_expected_band(fk, cohort_key_mon)
                            if band is not None:
                                med, q25, q75 = band
                                x_band = np.asarray([c for c in x if int(c) in med.index], dtype=float)
                                if x_band.size > 0:
                                    ...
                                    fig_s.add_trace(go.Scatter(x=x_band, y=y_q25, mode='lines', fill='tonexty', line=dict(width=0), opacity=0.15, showlegend=False, hoverinfo='skip'))
                                    fig_s.add_trace(go.Scatter(x=x_band, y=y_med, mode='lines', line=dict(dash='dash'), showlegend=False, hoverinfo='skip'))
                        fig_s.update_layout(height=240, margin=dict(l=30, r=10, t=10, b=25), xaxis_title='Cycle', yaxis_title=label)

                    fig_s.add_vline(x=float(current_cycle), line_width=2, line_dash='dash', line_color='red')
            ...
            for r in [1, 2, 3]:
                fig_mon.add_vline(x=float(current_cycle), line_width=2, line_dash="dash", line_color="red", row=r, col=1)
            ...
            if (z_dcr is not None) or (z_cap is not None):
                ...
                fig_z.add_hline(y=-3.0, line_width=1, line_dash="dash")
                # current cursor (현재 선택 cycle)
                fig_z.add_vline(x=float(current_cycle), line_width=2, line_dash="dash", line_color="red")
            ...
            # 4) 결과 표시
            # reference cycle: onset이 있으면 onset, 없으면 current (driver/리포트 기준점)
            ...
            # 5) '왜?' driver 후보 계산 (이상 유무와 무관하게 'early stress'로도 활용 가능)
            driver_candidates = [
                "thermal_stress",
                "temperature_mean",
                "temp_rise_cycle",
            ...
            # 6) 상태 배너 + 메시지
            if len(issues) == 0:
                # KPI(DCR/Capacity) 기준으로는 아직 큰 이탈이 없지만, driver 피처가 outlier일 수 있음
                sig_drivers = [d for d in drivers if np.isfinite(d.get("z", np.nan)) and abs(float(d["z"])) >= 3.0]
                if len(sig_drivers) == 0:
                    st.success("현재 선택된 배터리는 reference cohort의 기대 범위 내에서 큰 이탈이 관측되지 않았습니다.")
                else:
                    st.info(
                        "핵심 KPI(DCR/Capacity%) 기준의 '큰 이탈'은 아직 없지만, "
                        "일부 스트레스 신호(driver)가 cohort 대비 outlier 입니다. (early warning)"
                    )
            else:
            ...
            # 7) driver 리스트 표시 (이상/early warning 둘 다에서 보이게)
            if len(drivers) > 0:
                st.markdown(f"**Potential drivers around cycle {ref_cycle} (cohort 대비 robust z-score)**")
                for d in drivers:
                    st.markdown(
            ...
            # 8) 리포트 다운로드 (Markdown)
            report_lines = []
            report_lines.append(f"# Battery anomaly report — {selected_bid}")
            report_lines.append(f"- current_cycle: {int(current_cycle)}")
            report_lines.append(f"- reference_cohort: {cohort_label_mon} ({cohort_key_mon})")
            report_lines.append(f"- online_mode: {bool(online_mode_mon)}")
            report_lines.append("")
            report_lines.append("## RUL")
            if not math.isnan(current_pred_rul):
                report_lines.append(f"- predicted RUL(cycles): {current_pred_rul:.2f} (future region only)")
            else:
                report_lines.append("- predicted RUL(cycles): (예측 없음; prefix 구간)")
            report_lines.append("")
            report_lines.append("## Findings (rule + robust z-score)")
            if len(issues) == 0:
                report_lines.append("- No major deviation detected by KPI thresholds (DCR/Capacity%).")
            else:
                for name, onset_cycle, score in issues:
                    onset_txt = f"cycle {onset_cycle}" if onset_cycle is not None else "(onset unknown)"
                    report_lines.append(f"- {name} | onset={onset_txt} | robust z={score:.2f}")
            report_lines.append("")
            report_lines.append(f"## Potential drivers around cycle {ref_cycle}")
            if len(drivers) == 0:
                report_lines.append("- (insufficient cohort stats)")
            else:
                for d in drivers:
                    report_lines.append(f"- {d['tag']} ({d['feature']}={d['value']:.4g}, z={d['z']:.2f}) — {d.get('action','')}")
            report_lines.append("")
            report_lines.append("> Note: This is a demo heuristic. In production, thresholds should be tuned with cohort definition, sensor QA, and physics constraints.")
            report_md = "\n".join(report_lines)

            st.download_button(
                "📄 Anomaly report 다운로드 (Markdown)",
                data=report_md.encode("utf-8"),
                file_name=f"anomaly_report_{selected_bid}_cycle{int(current_cycle)}.md",
                mime="text/markdown",
            )
...
with st.expander("🆚 Compare batteries (Geotab-style)", expanded=False):
    ...
    if df_feat is None:
        ...
        fig_cmp = go.Figure()
        fig_cmp.add_vline(x=float(current_cycle), line_width=2, line_dash="dash", line_color="red")
...
with bottom_left:
    ...
    if FEATURE_STATS is None:
        ...
        if math.isnan(current_pred_rul):
            st.info("현재 사이클은 예측 구간이 아닙니다. (예측 없음)\n\n왼쪽 슬라이더를 r_ratio 이후 구간으로 옮기면 Scenario 계산이 활성화됩니다.")
        else:
            st.markdown("#### Baseline vs Scenario (what-if conditions)")
            st.caption(
