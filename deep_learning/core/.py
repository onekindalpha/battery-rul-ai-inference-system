...
# -------------------------------------------------
# Small UX/perf helpers (prevents "white/blurred freeze" on frequent reruns)
# -------------------------------------------------
...
def _recent_user_action(window_s: float = 1.2) -> bool:
...
def _align_1d(arr, n: int, name: str) -> np.ndarray:
    ...
    # pad/trim to match cycles length (keeps app alive instead of crashing)
    out = np.full(n, np.nan, dtype=float)
    m = min(n, arr.size)
    out[:m] = arr[:m]

    # warn only once per session (repeated warnings slow down reruns + cause flicker)
    warn_key = f"_warn_align_{name}"
...
def _schedule_rerun_js(interval_ms: int) -> None:
    """Client-side scheduled rerun without extra dependency.
    Tries Streamlit postMessage protocol first; falls back to iframe reload if needed.
    """
    nonce = int(st.session_state.get("_js_rerun_nonce", 0)) + 1
    st.session_state["_js_rerun_nonce"] = nonce
...
def _maybe_autoplay_advance(min_cycle: int, max_cycle: int, interval_ms: int) -> None:
    """Advance play_cycle at most once per interval. Safe against widget-triggered reruns."""
    if max_cycle <= min_cycle:
        return
    now = time.time()
    last = float(st.session_state.get("_autoplay_last_advance_ts", 0.0) or 0.0)
    interval_s = max(0.2, float(interval_ms) / 1000.0)
    if (now - last) >= interval_s:
        st.session_state["_autoplay_last_advance_ts"] = now
        pc = int(st.session_state.get("play_cycle", min_cycle))
        st.session_state["play_cycle"] = min(max_cycle, pc + 1)
...
def load_battery_cycle_max() -> Dict[str, float]:
    """battery_id -> max(cycle) from the cycle feature table (computed once per session)."""
    df = load_cycle_features()
    if df is None or df.empty:
        return {}
    try:
        s = df.groupby("battery_id")["cycle"].max()
        return {str(k): float(v) for k, v in s.items() if pd.notna(v)}
    except Exception:
        return {}

@st.cache_resource(show_spinner=False)
def load_cycle_features_indexed():
    """Indexed view for fast lookup: df.loc[(battery_id, cycle)]"""
    df = load_cycle_features()
    if df is None or df.empty:
        return None
    try:
        return df.set_index(["battery_id", "cycle"]).sort_index()
    except Exception:
        return None
...
class CycleFeatureContext:
    ...
    def __init__(self, battery_id: str, cycle: int):
        ...
        df = load_cycle_features_indexed()
        if df is not None:
            try:
...
if cycles.size == 0:
    st.error("선택한 record에 cycles 데이터가 없습니다. (빈 파일/로딩 오류 가능)")
    st.stop()
...
default_split = float(cycles[int(n_cycles * 0.5)]) if n_cycles > 0 else float(finite_cycles[0])
split_cycle = float(rec.get("split_cycle", default_split))
split_idx = _safe_nearest_idx(cycles, split_cycle)
...
min_cycle = int(finite_cycles.min())
max_cycle = int(finite_cycles.max())
...
# --- interaction tracking (pause autoplay briefly on user input) ---
# NOTE: This avoids autoplay "tick" fighting slider/checkbox interactions and causing UI freeze.
_prev_slider = st.session_state.get("_prev_cycle_slider")
if _prev_slider is None:
    st.session_state["_prev_cycle_slider"] = int(current_cycle_slider)
elif int(_prev_slider) != int(current_cycle_slider):
    st.session_state["_prev_cycle_slider"] = int(current_cycle_slider)
    _mark_user_action()
...
if _prev_autoplay is None:
    ...
    st.session_state["_prev_auto_play"] = _cur_autoplay
    _mark_user_action()
    # When turning autoplay ON, sync starting point to the slider once
    if _cur_autoplay:
        st.session_state["play_cycle"] = int(current_cycle_slider)
        st.session_state["_autoplay_last_advance_ts"] = time.time()

# Page value is stored later by the tab radio (key="page"). Fallback to previous/default.
_page_now = st.session_state.get("page", "Overview")

AUTOPLAY_INTERVAL_MS = 2500
AUTOPLAY_PAUSE_AFTER_UI_S = 0.8

if st.session_state.get("auto_play", False):
    # Keep scheduling reruns while autoplay is ON so it can resume after a short pause.
    if _page_now in ("Overview", "Anomaly"):
        if st_autorefresh is not None:
            st_autorefresh(interval=AUTOPLAY_INTERVAL_MS, key="autoplay_refresh")
        else:
            _schedule_rerun_js(AUTOPLAY_INTERVAL_MS)

        # Only advance after the user has been idle for a bit.
        if (not _recent_user_action(AUTOPLAY_PAUSE_AFTER_UI_S)) and int(st.session_state.get("play_cycle", min_cycle)) < max_cycle:
            _maybe_autoplay_advance(min_cycle, max_cycle, AUTOPLAY_INTERVAL_MS)

        # Auto-stop at the end (prevents endless reruns when already at max)
        if int(st.session_state.get("play_cycle", min_cycle)) >= max_cycle:
...
# -------------------------------------------------
all_true = rul_true
idx_slider = _safe_nearest_idx(cycles, current_cycle)
...
if in_future_region and fut_rul_pred.size > 0:
    idx_pred = _safe_nearest_idx(fut_cycles, current_cycle)
...
if (
    ...
    idx_pred_curr = _safe_nearest_idx(fut_cycles, current_cycle)
...
cycle_max_map = load_battery_cycle_max()
total_cycle_life = float(cycle_max_map.get(str(selected_bid), cycle_life_obs))
...
@st.cache_resource(show_spinner=False)
def load_cycle_features_with_cap_pct():
    """Cycle feature table with cap_pct precomputed (used by Anomaly/Compare)."""
    df = load_cycle_features()
    if df is None or df.empty:
        return None
    df2 = df.copy()
    return _compute_cap_pct(df2)
...
# Load full cycle-features only on tabs that need them (avoids heavy reruns/flicker)
df_feat_all = None
if page in ("Anomaly", "Compare / Fleet"):
    df_feat_all = load_cycle_features_with_cap_pct()
...
if page == "Overview":
    ...
    with right:
        ...
        # Build/reuse a base figure per (battery, r_ratio) so slider/toggle reruns stay fast.
        _fig_cache = st.session_state.setdefault("_rul_fig_base_cache", {})
        _fig_key = (str(selected_bid), float(current_r_ratio))
        _base_dict = _fig_cache.get(_fig_key)
        ...
        if _base_dict is None:
            ...
            # 2) Prediction + uncertainty (entire future region)
            if fut_cycles.size > 0 and fut_rul_pred.size > 0:
                fut_x = fut_cycles
                fut_pred_y = fut_rul_pred
                fut_std_y = (
                    fut_rul_std
                    if fut_rul_std.size == fut_rul_pred.size
                    else np.full_like(fut_pred_y, np.nan)
                )
            ...
            # Split marker (where prediction begins)
            try:
                if math.isfinite(float(split_cycle)):
                    _base.add_vline(
                        x=float(split_cycle),
                        line_width=1,
                        line_dash="dot",
            ...
            _base.update_layout(
                ...
                height=520,
                uirevision=f"rul-{selected_bid}-{current_r_ratio}",
            )

            _base_dict = _base.to_dict()
            _fig_cache[_fig_key] = _base_dict

        fig = go.Figure(_base_dict)

        # Cursor vline (changes every rerun)
        fig.add_vline(
        ...
        fig.update_layout(uirevision=f"rul-{selected_bid}-{current_r_ratio}")
