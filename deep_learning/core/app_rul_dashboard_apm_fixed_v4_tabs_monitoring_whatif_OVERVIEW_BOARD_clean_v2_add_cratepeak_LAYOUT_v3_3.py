...
# --- Load / Voltage ---
            ...
            with c1:
                _metric_card(
                    ...
                    f"{capacity_ahr_v:.3f} Ah" if np.isfinite(capacity_ahr_v) else "N/A",
                )
            with c2:
                _metric_card(
                    "soh<br>(capacity(% initial))",
                    f"{(soh_v*100.0):.1f} %" if np.isfinite(soh_v) else "N/A",
                )
            ...
            with t1:
                _metric_card(
                    ...
                    (f"{temp_start_c_v:.2f}°C" if np.isfinite(temp_start_c_v) else "N/A"),
                )
            with t2:
                _metric_card(
                    "temp_rise_cycle<br>(Tmax - Temperature_measured[0], °C)",
                    (f"{temp_rise_cycle_v:.2f}°C" if np.isfinite(temp_rise_cycle_v) else "N/A"),
                )
