...
def _pick_cycle_features_path(cands):
    ...
    def score(p):
        try:
            cols = set(pd.read_csv(p, nrows=5).columns)
...
AUTO_PLAY_DELAY_SEC = 1.5   # 자동 재생 프레임 간 간격 (초)
...
# Reduce flicker during autoplay reruns (hide Streamlit status widget)
st.markdown("""
<style>
...
auto_play_delay_sec = st.sidebar.slider("자동 재생 속도 (초)", min_value=0.8, max_value=3.0, value=float(st.session_state.get("auto_play_delay_sec", AUTO_PLAY_DELAY_SEC)), step=0.1, key="auto_play_delay_sec")
...
with tab_overview:
    ...
    st.caption("현재 선택된 사이클에서의 cycle-level 피처 값")

    # current-cycle feature row
    ...
    # --- Capacity ---
    capacity_ahr_v = _val(["capacity_ahr", "capacity_mean"])
    soh_v = _val(["soh"])
    capacity_derivative_v = _val(["capacity_derivative"])
    lli_v = _val(["lli"])

    # --- Load / Voltage ---
    current_mean_v = _val(["current_mean"])
    current_max_v = _val(["current_max"])
    current_min_v = _val(["current_min"])
    voltage_measured_mean_v = _val(["voltage_measured_mean"])
    voltage_min_v = _val(["voltage_min"])
    eff_c_rate_v = _val(["eff_c_rate"])
    discharge_time_sec_v = _val(["discharge_time_sec"])
    c_ref_ahr_v = _val(["c_ref_ahr"])
    c_rate_peak_v = _val(["c_rate_peak"])
    c_rate_mean_v = _val(["c_rate_mean"])
    eff_c_rate_C_v = _val(["eff_c_rate_C"])
    discharge_current_class_v = (str(row["discharge_current_class"]) if (row is not None and "discharge_current_class" in row and pd.notna(row["discharge_current_class"])) else "N/A")
    ...
    if (not np.isfinite(c_rate_mean_v)) and np.isfinite(current_mean_v) and np.isfinite(c_ref_ahr_v) and c_ref_ahr_v > 0:
        c_rate_mean_v = abs(current_mean_v) / c_ref_ahr_v
    if (not np.isfinite(eff_c_rate_C_v)) and np.isfinite(eff_c_rate_v) and np.isfinite(c_ref_ahr_v) and c_ref_ahr_v > 0:
        eff_c_rate_C_v = eff_c_rate_v / c_ref_ahr_v
    if discharge_current_class_v in ["N/A", "nan", "None"] and np.isfinite(current_min_v):
        a = abs(current_min_v)
        if 0.7 <= a <= 1.3:
    ...
    # --- Temperature ---
    temp_start_v = _val(["temp_measured_0_c", "ambient_temp_c"])
    temperature_measured_max_v = _val(["temperature_measured_max"])
    temperature_min_v = _val(["temperature_min"])
    temp_rise_cycle_v = _val(["temp_rise_cycle"])
    if (not np.isfinite(temp_rise_cycle_v)) and np.isfinite(temperature_measured_max_v) and np.isfinite(temp_start_v):
        temp_rise_cycle_v = temperature_measured_max_v - temp_start_v
    thermal_stress_v = _val(["thermal_stress", "thermal_stresss"])
    exp_cond = (str(row["experiment_condition"]) if (row is not None and "experiment_condition" in row and pd.notna(row["experiment_condition"])) else None)
    if exp_cond is None:
        if np.isfinite(temp_start_v):
            exp_cond = "저온 4°C" if temp_start_v <= 10.0 else ("실온" if 15.0 <= temp_start_v <= 35.0 else f"{temp_start_v:.1f}°C")
        else:
            exp_cond = "N/A"

    # --- Resistance ---
    dcr_re_v = _val(["dcr"])
    dcr_growth_v = _val(["dcr_growth"])
    lam_v = _val(["lam"])

    st.markdown("#### Capacity")
    c1, c2, c3, c4 = st.columns(4, gap="small")
    with c1:
        _metric_card("capacity_ahr (end-of-discharge capacity, Ah)", f"{capacity_ahr_v:.3f}" if np.isfinite(capacity_ahr_v) else "—")
    with c2:
        _metric_card("soh(capacity(% initial)", f"{soh_v:.3f}" if np.isfinite(soh_v) else "—")
    with c3:
        _metric_card("capacity_derivative = diff(C)/사이클 간 용량 변화량(ΔCapacity)", f"{capacity_derivative_v:.4f}" if np.isfinite(capacity_derivative_v) else "—")
    with c4:
        _metric_card("lli = slope(dC, dV)/Voltage sensitivity to capacity loss (proxy)", f"{lli_v:.4f}" if np.isfinite(lli_v) else "—")
    st.caption("참고 정의(툴팁/설명용): C = causal_roll(capacity_ahr, W_SMOOTH)")

    st.markdown("#### Load/Voltage")
    l1, l2, l3, l4, l5, l6 = st.columns(6, gap="small")
    with l1:
        _metric_card("current_mean", f"{current_mean_v:.3f}" if np.isfinite(current_mean_v) else "—")
    with l2:
        _metric_card("current_max", f"{current_max_v:.3f}" if np.isfinite(current_max_v) else "—")
    with l3:
        _metric_card("voltage_measured_mean", f"{voltage_measured_mean_v:.3f}" if np.isfinite(voltage_measured_mean_v) else "—")
    with l4:
        _metric_card("voltage_min", f"{voltage_min_v:.3f}" if np.isfinite(voltage_min_v) else "—")
    with l5:
        _metric_card("eff_c_rate(capacity_ahr / (discharge_time_sec/3600) (단위 A))", f"{eff_c_rate_v:.3f} A" if np.isfinite(eff_c_rate_v) else "—")
    with l6:
        _metric_card("discharge_time_sec", f"{discharge_time_sec_v:.0f} s" if np.isfinite(discharge_time_sec_v) else "—")

    l7, l8, l9, l10, l11 = st.columns(5, gap="small")
    with l7:
        _metric_card("c_ref_ahr (battery별 초기 용량 기준값)", f"{c_ref_ahr_v:.3f}" if np.isfinite(c_ref_ahr_v) else "—")
    with l8:
        _metric_card("c_rate_peak = abs(current_min) / c_ref_ahr", f"{c_rate_peak_v:.3f}" if np.isfinite(c_rate_peak_v) else "—")
    with l9:
        _metric_card("(원하면) c_rate_mean = abs(current_mean) / c_ref_ahr", f"{c_rate_mean_v:.3f}" if np.isfinite(c_rate_mean_v) else "—")
    with l10:
        _metric_card("(원하면) eff_c_rate_C = eff_c_rate / c_ref_ahr", f"{eff_c_rate_C_v:.3f}" if np.isfinite(eff_c_rate_C_v) else "—")
    with l11:
        _metric_card("discharge_current_class (1A/2A/4A)", discharge_current_class_v)

    st.markdown("#### Temperature")
    t1, t2, t3 = st.columns(3, gap="small")
    with t1:
        _metric_card("실험조건 (실온/4도)", exp_cond)
    with t2:
        _metric_card("(Temperature_measured[0] at discharge start, °C)", f"{temp_start_v:.2f}°C" if np.isfinite(temp_start_v) else "—")
    with t3:
        _metric_card("temperature_measured_max", f"{temperature_measured_max_v:.2f}°C" if np.isfinite(temperature_measured_max_v) else "—")

    t4, t5, t6 = st.columns(3, gap="small")
    with t4:
        _metric_card("temp_rise_cycle (temperature_measured_max - Temperature_measured[0], °C)", f"{temp_rise_cycle_v:.2f}°C" if np.isfinite(temp_rise_cycle_v) else "—")
    with t5:
        _metric_card("thermal_stresss (exp(alpha * (Tmax - Tref)/초기 대비 Tmax가 얼마나 올라갔는지)", f"{thermal_stress_v:.3f}" if np.isfinite(thermal_stress_v) else "—")
    with t6:
        _metric_card("temperature_min", f"{temperature_min_v:.2f}°C" if np.isfinite(temperature_min_v) else "—")

    st.markdown("#### Resistance")
    r1, r2, r3 = st.columns(3, gap="small")
    with r1:
        _metric_card("dcr(Re)", f"{dcr_re_v:.4f}" if np.isfinite(dcr_re_v) else "—")
    with r2:
        _metric_card("dcr growth(Internal resistance increase (log ratio vs early life)", f"{dcr_growth_v:.4f}" if np.isfinite(dcr_growth_v) else "—")
    with r3:
        _metric_card("lam = slope(dC, dRe)/Resistance sensitivity to capacity loss (proxy)", f"{lam_v:.4f}" if np.isfinite(lam_v) else "—")
...
if st.session_state.get("auto_play", False):
    ...
    if st_autorefresh is not None:
        tick = st_autorefresh(interval=int(float(st.session_state.get('auto_play_delay_sec', AUTO_PLAY_DELAY_SEC)) * 1000), key="autoplay_refresh")
        ...
        if cur < max_cycle:
            ...
            time.sleep(float(st.session_state.get('auto_play_delay_sec', AUTO_PLAY_DELAY_SEC)))
