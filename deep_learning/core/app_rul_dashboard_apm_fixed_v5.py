...
DRIVER_TAGS = {
    ...
    "dTdt_max": ("온도 변동(스윙) 급증", "열관리/센서 점검·운행 제한"),
}
...
st.sidebar.header("2) 배터리 세트 선택")

all_bids = sorted(records.keys())

# 포트폴리오 기본 세트 (있으면 우선)
PORTFOLIO_DEFAULT = ["B0018", "B0043", "B0055", "B0034", "B0033"]
default_bids = [b for b in PORTFOLIO_DEFAULT if b in all_bids]
if not default_bids:
    # fallback: 상위 4개
    default_bids = all_bids[:4]

analysis_bids = st.sidebar.multiselect(
    "분석/비교 배터리 세트",
    options=all_bids,
    default=default_bids,
    help="이 세트가 아래 Compare/Anomaly/리포트 기준이 됩니다.",
)

if not analysis_bids:
    analysis_bids = [all_bids[0]]

# 대표 배터리(상세 모니터링)
selected_bid = st.sidebar.selectbox(
    "대표 배터리 (상세 모니터링)",
    options=analysis_bids,
    index=0,
)
...
st.sidebar.caption("※ 현재 사이클 슬라이더를 r_ratio 이후 구간으로 옮기면 Scenario 계산이 활성화됩니다.")
...
st.sidebar.subheader("모델 예측 오차 지표 (검증용)")
if not math.isnan(rmse) or not math.isnan(mae):
    # 운영 화면에서는 핵심이 아니므로, 과한 강조/색을 피하고 작은 텍스트로만 표시
    parts = []
    if not math.isnan(rmse):
        parts.append(f"RMSE(예측구간): {rmse:.2f}")
    if not math.isnan(mae):
        parts.append(f"MAE(예측구간): {mae:.2f}")
...
# 🔹 4개 카드: 현재 사이클 / 관측 RUL / 예측 RUL / 상태(작은 카드)
    ...
    with h3:
        # st.metric는 텍스트가 과하게 커져서, 작은 카드로 통일
        if not math.isnan(current_pred_rul):
            val_txt = f"{current_pred_rul:.1f}"
            sub_txt = "EOL까지 남은 사이클(예측)"
        else:
            val_txt = "—"
            sub_txt = "예측 없음(초기 관측 구간)"
        st.markdown(
            f"""
            <div style='border-radius:10px; border:1px solid #e0e0e0;
                        padding:8px 10px; text-align:center;'>
                <div style='font-size:11px; color:#777; margin-bottom:2px;'>{sub_txt}</div>
                <div style='font-size:22px; font-weight:700; color:#222;'>{val_txt}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
with st.expander("📈 Degradation monitoring (selected battery)", expanded=True):
    ...
    if df_feat is None:
        ...
        if df_b.empty:
            ...
            # 7) driver 리스트 표시
            show_all_drivers = st.checkbox('driver(근거) 전체 보기', value=False, help='기본은 이상/early warning일 때만 표시합니다.')
            if (len(issues) > 0) or (len(sig_drivers) > 0) or show_all_drivers:
                if len(drivers) > 0:
            ...
            report_lines.append(f"- observed RUL(cycles): {current_obs_rul:.2f}")
            if not math.isnan(current_pred_rul):
...
with st.expander("🆚 Compare batteries", expanded=False):
    df_feat = load_cycle_features()
    if df_feat is None:
...
with bottom_left:
    ...
    if FEATURE_STATS is None:
        ...
        if st.session_state.get("auto_play", False):
            ...
            st.info("현재 사이클은 예측 구간이 아닙니다. (예측 없음)\n\n현재 사이클 슬라이더를 r_ratio 이후 구간으로 옮기면 Scenario 계산이 활성화됩니다.")
        else:
