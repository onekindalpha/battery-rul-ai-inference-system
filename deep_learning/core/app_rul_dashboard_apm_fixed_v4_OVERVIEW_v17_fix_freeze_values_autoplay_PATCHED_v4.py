...
# -------------------------------------------------
# UX/perf helpers (reduce 'white/blurred freeze' on rerun)
# -------------------------------------------------
def _mark_user_action() -> None:
    # Any widget interaction should pause autoplay refresh briefly
    st.session_state["_last_user_action_ts"] = time.time()

def _recent_user_action(window_s: float = 0.6) -> bool:
    ts = float(st.session_state.get("_last_user_action_ts", 0.0) or 0.0)
    return (time.time() - ts) < float(window_s)

def _safe_nearest_idx(x: np.ndarray, v: float) -> int:
    x = np.asarray(x, dtype=float).reshape(-1)
    if x.size == 0:
        return 0
    diff = np.abs(x - float(v))
    if np.all(np.isnan(diff)):
        return 0
    return int(np.nanargmin(diff))

def _align_1d(arr, n: int, name: str) -> np.ndarray:
    ...
    m = min(n, arr.size)
    out[:m] = arr[:m]
    st.warning(f"[data shape] {name} length({arr.size}) != cycles length({n}) → pad/trim applied.")
    return out

def _js_rerun_after(interval_ms: int, key: str = "js_rerun"):
    """Fallback rerun trigger without streamlit-autorefresh.
    Tries Streamlit postMessage rerun (works in many Streamlit builds).
    """
    try:
        ...
        components.html(
            ...
            <script>
              (function() {{
                ...
                const send = () => {{
                  const msgs = [
                    {{type: "streamlit:rerunScript", data: {{}}}},
                    {{type: "streamlit:rerun", data: {{}}}},
                    {{type: "streamlit:reload", data: {{}}}},
                  ];
                  for (const m of msgs) {{
...
def _on_manual_cycle_change():
    # live slider change → mark pending and timestamp (debounce heavy recompute)
    try:
        v = int(st.session_state.get("manual_cycle", st.session_state.get("play_cycle", 0)))
    except Exception:
        v = int(st.session_state.get("play_cycle", 0))
    st.session_state["_pending_cycle"] = v
    st.session_state["_pending_cycle_ts"] = time.time()
    _mark_user_action()

# ---- Optional autorefresh (used only when autoplay is ON) ----
# Autoplay needs a timer. If streamlit-autorefresh isn't installed, try installing ONCE per session.
if "_tried_install_autorefresh" not in st.session_state:
    st.session_state["_tried_install_autorefresh"] = False

try:
    from streamlit_autorefresh import st_autorefresh  # type: ignore
except Exception:
    st_autorefresh = None  # type: ignore
    if not bool(st.session_state.get("_tried_install_autorefresh", False)):
        st.session_state["_tried_install_autorefresh"] = True
        try:
            import subprocess
            subprocess.check_call([sys.executable, "-m", "pip", "install", "--quiet", "streamlit-autorefresh"])
            from streamlit_autorefresh import st_autorefresh  # type: ignore
        except Exception:
...
def load_cycle_features():
    ...
    성능 최적화:
    - (battery_id, cycle) MultiIndex를 만들어 단일 row 조회를 O(1)에 가깝게 만듭니다.
    """
    ...
    # MultiIndex (keep columns too)
    try:
        if not isinstance(df.index, pd.MultiIndex) or df.index.names != ["battery_id", "cycle"]:
            df = df.set_index(["battery_id", "cycle"], drop=False).sort_index()
    except Exception:
        pass
...
class CycleFeatureContext:
    ...
    def __init__(self, battery_id: str, cycle: int):
        ...
        if df is not None:
            try:
                sub = df.loc[(self.battery_id, self.cycle)]
                # MultiIndex lookup can return Series (single row) or DataFrame (duplicates)
...
cycles = np.asarray(rec.get("cycles", []), dtype=float).reshape(-1)
finite_cycles = cycles[np.isfinite(cycles)]
if cycles.size == 0:
    st.error("cycles 데이터가 비어있습니다. precomputed json을 확인하세요.")
    st.stop()
if finite_cycles.size == 0:
    st.error("cycles 값이 모두 NaN/inf 입니다. 데이터 로딩/전처리를 확인하세요.")
    st.stop()

n_cycles = int(cycles.size)
rul_true = _align_1d(rec.get("rul_true", []), n_cycles, "rul_true")
rul_pred = _align_1d(rec.get("rul_pred", []), n_cycles, "rul_pred")
rul_std  = _align_1d(rec.get("rul_std", [np.nan] * n_cycles), n_cycles, "rul_std")

capacity_curve = np.asarray(rec.get("capacity_curve", []), dtype=float).reshape(-1)
has_capacity = capacity_curve.size == cycles.size


split_cycle = rec.get("split_cycle", float(cycles[int(len(cycles) * 0.5)]))
split_idx = _safe_nearest_idx(cycles, float(split_cycle))
...
if st.session_state.selected_bid != selected_bid:
    ...
    st.session_state.pop("_autoplay_tick", None)
    st.session_state.pop("_autoplay_last_ts", None)
    _mark_user_action()
...
# autoplay 토글 변화(실제 사용자 입력) 감지
_prev_autoplay = st.session_state.get("_prev_auto_play")
_cur_autoplay = bool(st.session_state.get("auto_play", False))
if _prev_autoplay is None:
    st.session_state["_prev_auto_play"] = _cur_autoplay
elif bool(_prev_autoplay) != _cur_autoplay:
    st.session_state["_prev_auto_play"] = _cur_autoplay
    _mark_user_action()

AUTOPLAY_INTERVAL_MS = 1800  # 너무 짧으면 UI가 자주 '하얗게' 보여서 완화
AUTOPLAY_PAUSE_AFTER_UI_S = 0.6

if _cur_autoplay:
    # 자동재생 중에는 slider 대신 간단 표시로 렌더링 비용을 줄임
    pc = int(st.session_state.get("play_cycle", min_cycle))
    st.sidebar.write(f"현재 사이클: **{pc} / {max_cycle}**")
    if max_cycle > min_cycle:
        st.sidebar.progress((pc - min_cycle) / float(max_cycle - min_cycle))

    _page_now = st.session_state.get("page", "Overview")
    if _page_now not in ("Overview", "Anomaly"):
        st.sidebar.info("자동 재생은 Overview/Anomaly 탭에서만 동작합니다.")
    elif st_autorefresh is None:
        # fallback: JS postMessage rerun + time-based tick
        _js_rerun_after(AUTOPLAY_INTERVAL_MS, key="autoplay_js_rerun")
        now = time.time()
        last = float(st.session_state.get("_autoplay_last_ts", 0.0) or 0.0)
        if (now - last) >= (AUTOPLAY_INTERVAL_MS / 1000.0):
            st.session_state["_autoplay_last_ts"] = now
            if not _recent_user_action(AUTOPLAY_PAUSE_AFTER_UI_S):
                st.session_state["play_cycle"] = min(max_cycle, int(st.session_state.get("play_cycle", min_cycle)) + 1)
    else:
        _tick = st_autorefresh(interval=AUTOPLAY_INTERVAL_MS, key="autoplay_refresh")
        _prev_tick = st.session_state.get("_autoplay_tick")
        if _prev_tick is None:
            st.session_state["_autoplay_tick"] = _tick
        elif _tick != _prev_tick:
            st.session_state["_autoplay_tick"] = _tick
            if not _recent_user_action(AUTOPLAY_PAUSE_AFTER_UI_S):
                st.session_state["play_cycle"] = min(max_cycle, int(st.session_state.get("play_cycle", min_cycle)) + 1)

    current_cycle = int(st.session_state.get("play_cycle", min_cycle))
    heavy_ready = True

else:
    # 수동 모드: 기본은 LIVE (적용 버튼 없이). 대신 heavy 계산은 debounce로 늦춰서 '하얀 화면' 시간을 줄임.
    manual_apply_mode = st.sidebar.checkbox("수동 조작: 적용 버튼(느릴 때만)", value=False, key="manual_apply_mode")

    if manual_apply_mode:
        with st.sidebar.form("manual_cycle_form", clear_on_submit=False):
            manual_cycle = st.slider(
                "현재 사이클 (수동 모드)",
                min_value=min_cycle,
                max_value=max_cycle,
                step=1,
                value=int(st.session_state.get("play_cycle", max(min_cycle, 1))),
                key="manual_cycle_form_value",
            )
            submitted = st.form_submit_button("적용")
        if submitted:
            st.session_state["play_cycle"] = int(manual_cycle)
            st.session_state["_pending_cycle"] = None
            _mark_user_action()
        current_cycle = int(st.session_state.get("play_cycle", min_cycle))
        heavy_ready = True

    else:
        # LIVE slider: 변화가 빠르게 연속 발생하면, 무거운 섹션(피처/보드)을 잠시 스킵 후 안정되면 자동 갱신
        manual_cycle = st.sidebar.slider(
            "현재 사이클 (수동 모드)",
            min_value=min_cycle,
            max_value=max_cycle,
            step=1,
            value=int(st.session_state.get("_pending_cycle", st.session_state.get("play_cycle", max(min_cycle, 1)))),
            key="manual_cycle",
            on_change=_on_manual_cycle_change,
        )

        # pending 관리
        DEBOUNCE_S = 0.35
        pending = st.session_state.get("_pending_cycle", int(manual_cycle))
        pending_ts = float(st.session_state.get("_pending_cycle_ts", 0.0) or 0.0)

        # 슬라이더 값은 즉시 반영(표시용)
        current_cycle = int(pending)

        # debounce 창이 지나면 확정 (autorefresh가 없으면 즉시 확정)
        if st_autorefresh is None:
            st.session_state["play_cycle"] = int(pending)
            st.session_state["_pending_cycle"] = None
            heavy_ready = True
        elif pending is not None and (time.time() - pending_ts) >= DEBOUNCE_S:
            st.session_state["play_cycle"] = int(pending)
            st.session_state["_pending_cycle"] = None
            heavy_ready = True
        else:
            heavy_ready = False
            st.sidebar.caption("조작 중… 잠시 후 자동으로 갱신됩니다.")
            st_autorefresh(interval=int(DEBOUNCE_S * 1000), key="manual_debounce_refresh")
...
# -------------------------------------------------
all_true = rul_true
idx_slider = _safe_nearest_idx(cycles, float(current_cycle))
...
cycle_ctx = CycleFeatureContext(selected_bid, current_cycle if heavy_ready else int(st.session_state.get('play_cycle', current_cycle)))
...
if page == "Overview":
    ...
    with left:
        ...
        if in_future_region and fut_cycles.size > 0 and fut_rul_std.size > 0 and not math.isnan(current_pred_rul):
            try:
                idx_pi = _safe_nearest_idx(fut_cycles, float(current_cycle))
    ...
    with right:
        st.markdown("## RUL trajectory")        # Base figure is constant per (battery, r_ratio) → cache in session_state to cut rerun cost
        _rul_base_key = (str(selected_bid), float(st.session_state.get("r_ratio", DEFAULT_R_RATIO)))
        if st.session_state.get("_rul_base_key") != _rul_base_key or ("_rul_base_fig" not in st.session_state):
            base = go.Figure()
            # True RUL
            ...
            # Full prediction region (constant)
            if fut_cycles.size > 0 and fut_rul_pred.size > 0:
                base.add_trace(
                    go.Scatter(
                        x=fut_cycles,
                        y=fut_rul_pred,
                ...
                # Uncertainty band (±2σ) if available
                if fut_rul_std.size == fut_rul_pred.size and (not np.all(np.isnan(fut_rul_std))):
                    upper = fut_rul_pred + 2.0 * fut_rul_std
                    lower = fut_rul_pred - 2.0 * fut_rul_std
                    base.add_trace(
                        go.Scatter(
                            x=np.concatenate([fut_cycles, fut_cycles[::-1]]),
            ...
            # Split line (train/adapt boundary)
            try:
                base.add_vline(
                    x=float(split_cycle),
                    line_width=1,
            ...
            # EOL line (true) if we can infer
            try:
                if cycles.size > 0 and all_true.size == cycles.size:
                    eol_cycle = float(cycles[np.nanargmin(all_true)]) if np.any(np.isfinite(all_true)) else float("nan")
                    if np.isfinite(eol_cycle):
                        base.add_vline(
                            x=eol_cycle,
                            line_width=2,
                            line_dash="dash",
            ...
            st.session_state["_rul_base_key"] = _rul_base_key
            st.session_state["_rul_base_fig"] = base

        fig = copy.deepcopy(st.session_state["_rul_base_fig"])

        # Cursor / marker (lightweight updates)
        try:
            fig.add_vline(
                x=float(current_cycle),
                line_width=2,
                line_dash="solid",
                line_color="rgba(200,0,0,0.45)",
                annotation_text="cursor",
                annotation_position="top",
            )
        except Exception:
            pass

        # point at current prediction (if available)
        if not math.isnan(current_pred_rul):
            fig.add_trace(
                go.Scatter(
                    x=[float(current_cycle)],
                    y=[float(current_pred_rul)],
                    mode="markers",
                    name="current",
                    marker=dict(size=10, color="rgba(200,0,0,0.85)"),
                )
            )
    ...
    if not heavy_ready:
        st.info('조작 중에는 상세 피처 계산을 잠시 생략합니다. (슬라이더를 놓으면 자동 갱신)')
        ...
        if heavy_ready:
            try:
                ...
                if df_all is not None:
                    # MultiIndex → fast slice by battery_id
    ...
    with lv6:
        _metric_card("eff_c_rate<br>(capacity_ahr / (discharge_time_sec/3600))", f"{eff_c_rate_v:.3f} A" if np.isfinite(eff_c_rate_v) else "—")
    with lv7:
    ...
    lv10, lv11, lv12 = st.columns(3, gap="small")
    with lv10:
    ...
    t0, t1, t2, t3, t4 = st.columns(5, gap="small")
    with t0:
    ...
    with t4:
        _metric_card("thermal_stresss<br>(exp(alpha * (Tmax - Tref)))", f"{thermal_stress_v:.3f} (unitless)" if np.isfinite(thermal_stress_v) else "—")
