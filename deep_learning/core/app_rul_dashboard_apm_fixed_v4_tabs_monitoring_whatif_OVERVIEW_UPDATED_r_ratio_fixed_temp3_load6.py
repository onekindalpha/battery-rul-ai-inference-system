...
NASA_FEATURES_PATH_CANDIDATES = [
    FILE_DIR / "analysis" / "nasa_features_rul.csv",
    FILE_DIR / "analysis" / "battery_training_data_cleaned_final_causal_with_true_crate_v2_expcond_temp0.csv",
    FILE_DIR / "battery_training_data_cleaned_final_causal_with_true_crate_v2_expcond_temp0.csv",
]
NASA_FEATURES_PATH = next((p for p in NASA_FEATURES_PATH_CANDIDATES if p.exists()), NASA_FEATURES_PATH_CANDIDATES[0])
...
# --- Load / Voltage ---
            ...
            with c3:
                _metric_card(
                    "capacity_derivative<br>= diff(C)",
                    f"{capacity_derivative_v:.4f}" if np.isfinite(capacity_derivative_v) else "N/A",
                )

            st.markdown("#### Load/Voltage")
            # Row 1 (4 cols): voltage + current + class
            l1, l2, l3, l4 = st.columns(4)
            with l1:
                _metric_card(
                    "voltage_min",
                    f"{voltage_min_v:.3f}" if np.isfinite(voltage_min_v) else "N/A",
                )
            with l2:
                _metric_card(
                    "voltage_measured_mean",
                    f"{voltage_measured_mean_v:.3f}" if np.isfinite(voltage_measured_mean_v) else "N/A",
                )
            with l3:
                _metric_card(
                    "current_min",
                    f"{current_min_v:.3f}" if np.isfinite(current_min_v) else "N/A",
                )
            with l4:
                st.markdown(
                    f"""
            ...
            # Row 2 (3 cols): eff_c_rate + c_rate_peak + discharge_time_sec
            l5, l6, l7 = st.columns(3)
            with l5:
                _metric_card(
                    "eff_c_rate<br>(capacity_ahr / (discharge_time_sec/3600))",
                    (f"{eff_c_rate_v:.3f} A" if np.isfinite(eff_c_rate_v) else "N/A"),
                )
            with l6:
                _metric_card(
                    "c_rate_peak<br>= abs(current_min) / c_ref_ahr",
                    (f"{c_rate_peak_v:.3f}" if np.isfinite(c_rate_peak_v) else "N/A"),
                )
            with l7:
                _metric_card(
                    "discharge_time_sec",
                    (f"{discharge_time_sec_v:.0f}" if np.isfinite(discharge_time_sec_v) else "N/A"),
                )
            ...
            if df_b.empty:
                ...
                if not df_now.empty and z_items:
                    ...
                    for label, col in z_items:
                        ...
                        vals[label] = v
                    # small table
                    df_show = pd.DataFrame([vals])
                    # keep z(Capacity) on the left and z(DCR) on the right (if both exist)
                    preferred_cols = [c for c in ["z(Capacity)", "z(DCR)"] if c in df_show.columns]
                    other_cols = [c for c in df_show.columns if c not in preferred_cols]
                    df_show = df_show[preferred_cols + other_cols]
