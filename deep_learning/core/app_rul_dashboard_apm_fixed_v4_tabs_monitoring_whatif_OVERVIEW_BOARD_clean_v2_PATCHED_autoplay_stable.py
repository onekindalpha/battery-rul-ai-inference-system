...
AUTO_PLAY_DELAY_SEC = 1.2   # 자동 재생 프레임 간 간격 (초)
...
if not st.session_state.get("auto_play", False):
    st.session_state.pop("_autoplay_last_tick", None)
...
# Main content (tabs)
# =================================================
AUTOPLAY_LIGHT_MODE = bool(st.session_state.get("auto_play", False))
...
# --- Load / Voltage ---
            ...
            with l6:
                if np.isfinite(c_rate_peak_v):
                    _metric_card(
                        "c_rate_peak<br>= abs(current_min) / c_ref_ahr",
                        f"{c_rate_peak_v:.3f}",
                    )
            with l7:
                if np.isfinite(discharge_time_sec_v):
                    _metric_card(
                        "discharge_time_sec",
                        f"{discharge_time_sec_v:.0f}",
                    )
...
with tab_monitoring:
    if AUTOPLAY_LIGHT_MODE:
        st.info('Auto-play is running — Monitoring tab is paused to keep the page responsive.')
    else:
...
with tab_compare:
    if AUTOPLAY_LIGHT_MODE:
        st.info('Auto-play is running — Compare/Fleet tab is paused to keep the page responsive.')
    else:
...
with tab_whatif:
    if AUTOPLAY_LIGHT_MODE:
        st.info('Auto-play is running — What-if tab is paused to keep the page responsive.')
    else:
        st.markdown("## What-if")
...
if st.session_state.get("auto_play", False):
    ...
    if st_autorefresh is not None:
        ...
        st.warning("Auto-play requires streamlit-autorefresh. Disabling auto-play to avoid UI freeze.")
        st.session_state["auto_play"] = False
