...
with tab_overview:
    ...
    with left:
        ...
        if cycle_ctx.row is None:
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.metric("capacity_ahr (end-of-discharge capacity, Ah)", f"{capacity_ahr_v:.3f}" if np.isfinite(capacity_ahr_v) else "N/A")
            with c2:
                st.metric("soh (capacity(% initial))", f"{soh_v:.3f}" if np.isfinite(soh_v) else "N/A")
            ...
            with l2:
                st.metric("eff_c_rate (capacity_ahr / (discharge_time_sec/3600) (단위 A))", f"{eff_c_rate_v:.3f}" if np.isfinite(eff_c_rate_v) else "N/A")
            with l3:
                st.markdown(
                    f"""
                    <div style="border:1px solid #e6e6e6; border-radius:10px; padding:8px 10px; text-align:center;">
            ...
            with r2:
                st.metric("dcr growth (Internal resistance increase (log ratio vs early life))", f"{dcr_growth_v:.4f}" if np.isfinite(dcr_growth_v) else "N/A")
