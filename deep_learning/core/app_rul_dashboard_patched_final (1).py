...
# ❗ 이 문구 유지
st.sidebar.caption(
    "※ 모델을 통해 더 적은 초기 데이터만 보고 예측해 보고 싶다면 비율을 조금 낮춰 보세요 (예: 0.25 → 0.20)."
)
...
# 🔁 Auto-play 체크박스
# ✅ 최초 접속(세션 시작) 시 자동으로 ON
#    사용자가 체크 해제하면 같은 세션에서는 OFF 유지
if "auto_play" not in st.session_state:
    st.session_state["auto_play"] = True

auto_play = st.sidebar.checkbox("사이클 자동 재생", key="auto_play")
...
# =================================================
# 배터리 잔존 수명 궤적(시간에 따른 경로)
with top_right:
    st.markdown("### Battery RUL trajectory")
    fig = go.Figure()
...
# =================================================
with bottom_left:
    st.markdown("### Scenario Builder (what-if 시뮬레이션)")

    if st.session_state.get("auto_play", False):
        st.info("사이클 자동 재생 중에는 Scenario Builder가 정지됩니다.\n\n")
    else:
