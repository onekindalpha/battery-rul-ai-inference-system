...
if page == "Overview":
    ...
    with cap3:
        _metric_card("capacity_derivative = diff(C)<br>/사이클 간 용량 변화량(ΔCapacity)", f"{capacity_derivative_v:.5f}" if np.isfinite(capacity_derivative_v) else "—")
    st.caption("참고 정의(툴팁/설명용): C = causal_roll(capacity_ahr, W_SMOOTH)")

    # Load/Voltage
    st.markdown("#### Load/Voltage")
    lv1, lv2, lv3, lv4 = st.columns(4, gap="small")
    with lv1:
    ...
    lv9, lv11, lv12 = st.columns(3, gap="small")
    with lv9:
        _metric_card("c_rate_peak = abs(current_min)<br>/ c_ref_ahr", f"{c_rate_peak_v:.3f}" if np.isfinite(c_rate_peak_v) else "—")
    with lv11:
        _metric_card("eff_c_rate_C = eff_c_rate<br>/ c_ref_ahr", f"{eff_c_rate_C_v:.3f}" if np.isfinite(eff_c_rate_C_v) else "—")
    with lv12:
    ...
    with t1:
        _metric_card("실험조건<br>(실온/4도)", exp_cond_v if exp_cond_v else "—")
    with t2:
    ...
    with t4:
        _metric_card("temperature_min", f"{temp_min_v:.2f}°C" if np.isfinite(temp_min_v) else "—")

    t5, t6, t7 = st.columns(3, gap="small")
    with t5:
    ...
    with t6:
        _metric_card("thermal_stresss<br>(exp(alpha * (Tmax - Tref)/초기 대비 Tmax가 얼마나 올라갔는지)", f"{thermal_stress_v:.3f}" if np.isfinite(thermal_stress_v) else "—")
    with t7:
        st.empty()
    ...
    with r1:
        _metric_card("dcr(Re)", f"{dcr_v:.4f}" if np.isfinite(dcr_v) else "—")
    with r2:
