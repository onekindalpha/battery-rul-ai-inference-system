...
NASA_FEATURES_PATH_CANDIDATES = [
    # Prefer the enriched table (includes discharge_time_sec, c_ref_ahr, c_rate_peak, discharge_current_class, etc.)
    ...
    # Fallback (may miss some engineered columns)
...
# --- Load / Voltage ---
        ...
        if df_feat_all is None or df_feat_all.empty:
            ...
            if df_b.empty:
                ...
                # z(DCR) vs cycle median/IQR  + DCR ratio (vs early life)
                z_items = []
                ...
                # DCR ratio (vs early life): exp(dcr_growth) if available
                if "dcr_growth" in df_b.columns:
                    df_b = df_b.assign(dcr_ratio=np.exp(df_b["dcr_growth"].astype(float)))
                    z_items.insert(0, ("DCR ratio (vs early life)", "dcr_ratio"))

                # Show current-cycle values
# Show current-cycle z values
                ...
                if not df_now.empty and z_items:
                    ...
                    # keep DCR ratio on the left and z(DCR) on the right
                    preferred_cols = [c for c in ["DCR ratio (vs early life)", "z(DCR)"] if c in df_show.columns]
