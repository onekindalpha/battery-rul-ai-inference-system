...
if _cur_autoplay:
    ...
    if _page_now not in ("Overview", "Anomaly"):
        ...
        st.sidebar.error("자동 재생을 위해 streamlit-autorefresh가 필요합니다. 터미널에서: pip install streamlit-autorefresh")
    else:
    ...
    # 수동 모드: 기본은 '적용 버튼'으로 해서 드래그 중 rerun을 막음(흰 화면 감소)
    manual_apply_mode = st.sidebar.checkbox("수동 조작: 적용 버튼(권장)", value=True, key="manual_apply_mode")
    ...
    if manual_apply_mode:
        ...
        manual_cycle = st.sidebar.slider(
            ...
            step=1,
            value=int(st.session_state.get("play_cycle", max(min_cycle, 1))),
            key="manual_cycle_slider_live",
            on_change=_mark_user_action,
        )
        st.session_state["play_cycle"] = int(manual_cycle)
