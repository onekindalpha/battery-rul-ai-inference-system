...
function metric(valuesA: number[], valuesB: number[], kind: 'rmse' | 'mae') {
  const pairs = valuesA.map((a, i) => [a, valuesB[i]]).filter(([a, b]) => Number.isFinite(a) && Number.isFinite(b))
  if (!pairs.length) return NaN
  if (kind === 'mae') return pairs.reduce((s, [a, b]) => s + Math.abs(a - b), 0) / pairs.length
  return Math.sqrt(pairs.reduce((s, [a, b]) => s + Math.pow(a - b, 2), 0) / pairs.length)
}

export function ExplainabilityCleanV23({
  battery,
  rRatio,
  cycle,
}: {
    ...
    async function run() {
      ...
      try {
        const [pRaw, s] = await Promise.all([
          ...
          getJSON(`/api/fixed4/shap-v23`).catch(() => null),
        ])
        if (!alive) return
  ...
  const fullRmseRaw = n(pre?.metrics?.rmse)
  const fullMaeRaw = n(pre?.metrics?.mae)
  const fullRmse = Number.isFinite(fullRmseRaw) ? fullRmseRaw : metric(pre?.truth ?? [], pre?.pred ?? [], 'rmse')
  const fullMae = Number.isFinite(fullMaeRaw) ? fullMaeRaw : metric(pre?.truth ?? [], pre?.pred ?? [], 'mae')
  ...
  return (
    <div style={{ overflowY: 'auto', paddingRight: 4 }}>
      ...
      <p style={{ fontSize: 12, color: '#666', lineHeight: 1.55, marginTop: 0 }}>
        Prediction confidence, model architecture, and global model-level SHAP importance.
      </p>
      ...
      <div style={{ marginBottom: 16, padding: 12, background: '#f8fafc', border: '1px solid #e5e7eb', borderRadius: 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 8, marginTop: 8 }}>
          ...
          {card('Overall RMSE', Number.isFinite(fullRmse) ? fullRmse.toFixed(2) : '평가 전')}
          {card('Overall MAE', Number.isFinite(fullMae) ? fullMae.toFixed(2) : '평가 전')}
      ...
      <div style={{ marginBottom: 16, padding: 12, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8 }}>
        ...
        <div style={{ fontSize: 12, lineHeight: 1.65, color: '#444' }}>
          <p style={{ marginTop: 0 }}><strong>Backbone:</strong> CEEMDAN–Transformer–DNN decomposes noisy capacity trajectories and models both regeneration/local components and global residual degradation trend.</p>
          ...
          <p style={{ marginBottom: 0 }}><strong>Uncertainty:</strong> The prediction interval is displayed from model predictive standard deviation, while RMSE/MAE summarize observed prediction error.</p>
        </div>
      </div>
